...
func TestSnapshotReuseFlowRefreshesExistingSnapshotWithSameToken(t *testing.T) {
	...
	server.SetTokenPages("token-one", []storagePageFixture{{
		Count:          1,
		ActivitiesJSON: `[{"id":"activity-1","date":"2024-01-01T10:00:00Z","type":"BUY","quantity":1,"valueInBaseCurrency":100,"unitPriceInAssetProfileCurrency":100,"baseCurrency":"USD","SymbolProfile":{"symbol":"BTC","name":"Bitcoin","currency":"USD","symbolProfileId":"asset-btc-reuse-001"}}]`,
	}})
	...
	server.SetTokenPages("token-one", []storagePageFixture{{
		Count:          1,
		ActivitiesJSON: `[{"id":"activity-2","date":"2025-01-01T10:00:00Z","type":"BUY","quantity":2,"valueInBaseCurrency":200,"unitPriceInAssetProfileCurrency":100,"baseCurrency":"USD","SymbolProfile":{"symbol":"BTC","name":"Bitcoin","currency":"USD","symbolProfileId":"asset-btc-reuse-001"}}]`,
	}})
...
func TestSnapshotReuseFlowCreatesIsolatedSnapshotForDifferentValidToken(t *testing.T) {
	...
	server.SetTokenPages("token-two", []storagePageFixture{{
		Count:          1,
		ActivitiesJSON: `[{"id":"activity-2","date":"2025-01-01T10:00:00Z","type":"BUY","quantity":2,"valueInBaseCurrency":200,"unitPriceInAssetProfileCurrency":100,"baseCurrency":"USD","SymbolProfile":{"symbol":"ETH","name":"Ether","currency":"USD","symbolProfileId":"asset-eth-reuse-001"}}]`,
	}})
	service := newTokenAwareSyncService(baseDir, server)
	config := mustSnapshotReuseConfig(t, server.URL())
	inspector := snapshotstore.NewEncryptedStore(baseDir, nil)
