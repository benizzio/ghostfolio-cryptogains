...
# Generate Code Standard Drift Remediation Plan

Append a drift remediation phase to the active feature `tasks.md` from the current code-standard drift report.
...
- Do not create `code-standard-drift-remediation.md` or any other separate remediation checklist.
...
3. Verify `FEATURE_DIR/code-standard-drift-report.md` and `FEATURE_DIR/tasks.md` exist. If the report is missing, stop and instruct the user to run `/speckit.coding-standards-drift-analysis.report` first.
...
5. Append a new final phase dedicated to code-standard drift remediation, following the phase structure used by the current `tasks.md` rather than a hard-coded template.
6. Create one unchecked Spec Kit task per selected drift finding:
   ...
   - reference `code-standard-drift-report.md` and the finding's report topic or anchor
...
- Keep task text actionable, implementation-oriented, and scoped to coding-standard remediation.
- Every generated task MUST reference its source `DRIFT-###` topic in `code-standard-drift-report.md`.
