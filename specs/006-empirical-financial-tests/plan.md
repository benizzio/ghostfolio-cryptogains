...
- Dependency tests or review checks prove external oracle materials include applicable license text, source provenance, supported artifact paths, upstream URL, version or commit identity, checksums, platform support, adapter constraints, no prohibited binary-only vendoring, no committed rotki source checkout, and the documented local Python adapter boundary.
