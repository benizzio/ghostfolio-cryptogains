...
- **Rotki-backed external oracle**: A pinned test-time adapter that derives FIFO, LIFO, HIFO, and Average Cost aggregate expected values from rotki source and records source provenance, version or commit identity, adapter constraints, arguments, and hashes.
...
- The empirical suite relies on repository-controlled external oracle artifacts or pinned adapter source instead of system installations.
