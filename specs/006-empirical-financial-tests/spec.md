...
**Input**: User description: "Create an empirical external dataset, ~~an hledger-backed empirical calculation oracle~~ a rotki-backed empirical calculation oracle for pure methods with a Scope-Local Hybrid composite oracle, and empirical solidified financial integration tests for the capital gains and losses calculation layer. This is internal test infrastructure only and does not define user-facing stories."

## Purpose

This specification defines the needs for creating and using an `empirical external dataset` to validate the financial calculation layer against expected outcomes produced by ~~an external plain-text-accounting engine~~ the selected external oracle or composite oracle.
...
- Q: How should empirical comparisons handle decimal equality versus precision differences? -> A: Quantities must compare by exact decimal equality after normalization. Calculated financial values must first be normalized under one selected decimal policy; if ~~hledger~~ the selected external oracle cannot be configured to match the project's 16-decimal production policy for every valid case, `GHOSTFOLIO_CRYPTOGAINS_REPORT_DECIMAL_POLICY` acts as an application-run-scoped override so empirical runs can make the project calculation layer use ~~hledger's~~ the selected external oracle's established decimal policy while normal production runs and current empirical runs keep the `scale=16,rounding=half_up` default when the variable is unset. Custom documented values are allowed when needed to align with an oracle, with a practical maximum scale of 64 for safety. After that alignment, calculated financial values may use documented per-field tolerances for residual ~~hledger/project~~ external-oracle/project deviations, capped at one unit of the selected decimal-policy scale.
- Q: How should empirical tests handle ~~hledger~~ external oracle availability? -> A: ~~hledger must be vendored inside the repository for empirical test use.~~ BUG-001 and BUG-002 supersede this with repository-controlled rotki provenance, documented adapter constraints, checksums, committed normalized fixtures, and verified untracked pinned rotki source execution only during explicit regeneration.
...
**Bugfix**: 2026-06-13 — [BUG-003] Removed obsolete hledger boundary from active oracle, fixture, dependency, and verification scope.

- Q: May hledger remain as a retained historical or auxiliary test-time boundary after BUG-002? -> A: No. Hledger is fully out of active feature scope after BUG-003, including vendoring, command wrappers, generated journals, normalization code, tests, fixture metadata, and documentation paths. Hledger references may remain only in historical bug reports or explicit strikethrough rationale that identifies the superseded assumption.
...
- **Rotki-backed external oracle**: A pinned test-time adapter implemented by `tools/empiricaloracle/rotki_adapter.py` that derives FIFO, LIFO, HIFO, and Average Cost aggregate expected values from verified rotki source downloaded or reused under `.cache/empiricaloracle/rotki-source/` during explicit golden-fixture regeneration, uses generated adapter inputs under `.cache/empiricaloracle/oracle-inputs/`, and records source provenance, version or commit identity, adapter constraints, arguments, and hashes.
...
4. The oracle supports the Scope-Local Hybrid (`scope_local_hybrid`) method by using ~~hledger-backed evidence~~ rotki-backed arithmetic assertions where an external subproblem is valid, and by persisting the remaining project-owned routing or lifecycle segments as explicit `project_composition_only` unsupported segments with documented reasons when no current committed `project_composition_rule` fixture assertion exists.
...
7. Active oracle materials do not include superseded-boundary command wrappers, generated journals, normalization paths, vendored source, executables, metadata, tests, or support labels; references to that boundary are permitted only in historical bug reports or explicit strikethrough rationale.
...
- **FR-027**: Scope-Local Hybrid empirical comparisons MUST use a separate composite oracle that currently compares rotki-backed arithmetic evidence and records the remaining project-owned routing or lifecycle segments as explicit `project_composition_only` unsupported segments with documented reasons.
...
- **FR-029**: The explicit golden-fixture regeneration command MUST download or reuse pinned rotki source in an untracked project-local folder, verify configured source URL, pinned version or commit, signed tag object, source checksum, and adapter constraints, and execute rotki calculation code through the project-owned local Python adapter boundary.
...
- **CMP-003**: Current Scope-Local Hybrid comparable evidence MUST label compared arithmetic evidence as `rotki_backed`. Remaining project-owned routing or lifecycle segments MUST be persisted as unsupported segments with comparison policy `project_composition_only` and a documented reason. `project_composition_rule` evidence remains reserved by the fixture model but is not required by the current committed fixtures.
...
- **FIN-004**: If ~~hledger~~ the selected external oracle cannot be configured or normalized to align with the 16-decimal internal precision for every otherwise valid empirical case, the implementation MUST add an application-run-scoped environment-variable override path for the project internal report-calculation decimal policy. The expected variable is `GHOSTFOLIO_CRYPTOGAINS_REPORT_DECIMAL_POLICY`; when it is unset, normal production runs and current empirical runs MUST keep the `scale=16,rounding=half_up` default. Custom documented values in the form `scale=<digits>,rounding=half_up` MAY be used when needed to align a given application run with the external-oracle-established decimal policy, with a practical maximum scale of 64 for safety.
...
- **DEP-009**: Hledger is out of active feature scope after BUG-003. The repository MUST remove hledger vendoring, generated journals, command wrappers, normalization code, provenance metadata, hledger-specific tests, fixture labels, and documentation references from active project artifacts, except historical bug reports or explicit strikethrough rationale.
...
- **SC-006**: Precision-sensitive cases match exactly under either the 16-decimal production default policy or an explicitly documented application-run-scoped decimal-policy override that matches ~~hledger's~~ the selected external oracle's established policy.
- **SC-007**: ~~Vendored hledger external-oracle materials include applicable license notices, corresponding source or provenance required by the applicable license, version or commit identity, source URL, checksums for source and any supported executable artifact, and empirical tests use external oracle tooling only as a separate test-time boundary.~~ Rotki provenance and adapter materials include applicable license notices, source provenance, version or commit identity, source URL, checksums for source verification and adapter inputs, adapter constraints, and empirical tests use oracle tooling only as a separate test-time boundary. Rotki source itself is not vendored; explicit regeneration verifies the pinned source download or reuse path, while normal fixture-backed empirical tests do not fetch rotki source. Hledger vendoring, journals, executables, and metadata are not retained after BUG-003 except historical bug reports or strikethrough rationale.
- **SC-008**: BUG-003 cleanup verification finds no active non-historical `hledger` or `hleger` references in source, tests, fixtures, active contracts, quickstart, README files, or oracle-output documentation after the cleanup tasks complete.
