...
3. **Given** a Detailed Per-Asset Audit Report row carries the inherited `IsZeroPricedHoldingReduction` classification, **When** Annex 1 is generated, **Then** its `Original Activity Currency` cell is blank while its `Calculation Currency` and all other applicable audit evidence remain visible.
4. **Given** a Detailed Per-Asset Audit Report row does not carry the inherited `IsZeroPricedHoldingReduction` classification, **When** Annex 1 is generated, **Then** the row retains its existing `Original Activity Currency` value.
...
- **FR-001**: The system MUST apply each presentation change in this specification to its explicitly stated Markdown and PDF boundary. The warning applies to the Markdown main document and the combined PDF main section, not the separate Markdown Annex. Financial formatting, quantity preservation, boolean labeling, audit-currency presentation, and converted-amount layout apply to every main-report or Annex boundary identified by FR-005 and the Financial Presentation Acceptance Matrix.
...
- **FR-004a**: The accepted financial-formatting input domain is every finite decimal whose adjusted base-10 exponent, defined as source exponent plus source coefficient digit count minus one, is between `-100000` and `100000` inclusive, whose correctly HALF UP quantized scale-2 result including any carry also has adjusted exponent no greater than `100000`, and whose required quantization precision fits the pinned `apd` implementation's signed 32-bit exponent arithmetic. Required precision is the source coefficient digit count plus any coefficient expansion needed to reach exponent `-2`, plus one possible carry digit, and MUST NOT exceed `2147383649`, derived from `apd.MinExponent - precision + 1 >= math.MinInt32`. Every value in this domain MUST produce an FR-004 string under FR-006, subject to later document-layout validation; a value outside it, including an upper-bound value whose rounding carry would produce adjusted exponent `100001`, MUST return the FR-022 unrepresentable-value error before visible output. Successful-display requirements, including FR-011 and the successful numeric vectors, apply only within this domain.
...
- **FR-013**: This feature MUST consume the existing `IsZeroPricedHoldingReduction` classification inherited from the explained zero-priced `SELL` rule in `specs/003-store-activity-data/spec.md` FR-017 and its reporting treatment in `specs/005-generate-gains-report/spec.md` FR-029 and FR-029a. It MUST NOT recompute, redefine, broaden, or otherwise alter that classification or sync admission. In a Detailed Per-Asset Audit Report row carrying the classification, the system MUST leave only the visible `Original Activity Currency` cell blank and MUST NOT infer the classification from a displayed price, activity label, rounded amount, currency value, or source monetary-field shape.
...
- **FR-015**: A row that does not carry the inherited `IsZeroPricedHoldingReduction` classification MUST retain its existing `Original Activity Currency` value, including when a non-zero unit price is small enough to display as `0.00`. This feature suppresses no other row.
...
- **FR-020**: For each semantic item present in both formats for the same report request, Markdown and PDF MUST agree on warning text, financial value, quantity, disclosed rate and metadata, boolean label, applicable currency value, and converted-entry sequence. Permitted differences are limited to Markdown syntax, PDF styling and pagination, page titles, and separate Markdown Annex file boundaries.
...
- **FR-028**: A successful selected-format export result, including success-with-warning after opener failure, MUST identify every saved file as a cleartext financial-data export, report every saved path, and instruct the user to delete all listed files to remove the exported data. The application MUST retain no additional cleartext copy, report history, reopen catalog, durable output-path state, or automatic re-ingestion after the result flow ends.
...
### Security, Persistence, And User Export Evidence

- **Application-Managed Persistence Impact**: This feature adds no application-managed state, cache, temporary cleartext report, report history, retained output-path metadata, reopen catalog, remote persistence, or automatic re-ingestion of generated files.
- **Cryptographic Storage Evidence**: N/A. This feature places no financial or person-linked data in application-managed persistence; protected snapshots retain their existing token-derived encryption, and qualifying final report files follow EXP-001 instead.
- **EXP-001 — Explicit User-Requested Export Boundary**: Markdown and PDF files are direct user-requested final exports to the local user-controlled Documents directory. They MAY contain contracted cleartext financial fields because every saved path is disclosed, owner-only mode `0600` is requested where supported, collisions never overwrite prior files, failed attempts remove only current-attempt paths, and the application retains no additional cleartext copy or later management beyond the requested operating-system open handoff. Every successful result MUST disclose the cleartext financial-data status and provide the FR-028 deletion guidance.
- **SEC-001 — Confidentiality**: Generated Markdown main documents, Markdown Annex documents, and combined PDFs MAY contain only their contracted report fields. No generated document, result or error text including wrapped or dependency-originated causes, diagnostic artifact or field, screenshot, documentation or test example, or committed or generated test fixture may contain any real Ghostfolio or security token, bearer or JWT value, token-derived verifier or key, other material reusable for authentication or decryption, raw protected-payload material, or unrelated real user financial data. Real user financial data MUST also be redacted from returned errors, logs, screenshots, documentation examples, and committed or generated fixtures; examples and fixtures MUST use synthetic financial data. Raw protected-payload material includes serialized encrypted snapshot or envelope bytes, decrypted serialized payload bytes or fragments, and reversible encodings of any of them. Project-owned boundaries MUST redact or suppress prohibited material before display, return, logging, persistence, or artifact capture. Clearly synthetic, non-reusable sentinels MAY be used only to verify redaction and MUST NOT be copied from user credentials, financial records, or protected payloads. Inherited mode-authorized modeled diagnostic fields retain their separately reviewed redaction rules and MUST NOT gain disclosure authority from the report-export boundary.
...
- **Security Review Scope**: Review must confirm that the new warning and presentation transformations do not disclose secrets or real financial data outside contracted export fields, replace missing values with misleading zeros, or weaken local report-file handling. It must also verify cleartext and saved-path disclosure, deletion guidance, failed-attempt cleanup, non-retention, and non-re-ingestion. The feature adds no authentication, authorization, network, or remote-storage surface.
...
- **Scale Regression**: The Performance Acceptance Definition governs the exact local workload, two-minute limits, independent format timers, successful non-empty local outputs, opener invocation, environment record, and performance-coverage isolation. Deterministic package, contract, and integration tests govern warning, financial, rate, conversion-entry, output-cardinality, heading, and PDF pagination/content assertions.
...
- **Zero-Priced Holding Reduction**: The exact pre-format `IsZeroPricedHoldingReduction` classification inherited from Feature 003 FR-017 and Feature 005 FR-029/FR-029a, not a classification defined or recomputed by this feature and not a conclusion drawn from visible `0.00`.
...
- `boolean/{true,false}`;
- `currency/{classified-zero-priced,unclassified-priced,unclassified-tiny-positive}`; and
...
- `Z`, classified zero-priced controls: every Annex activity occurrence in both formats supplied with `IsZeroPricedHoldingReduction` equal to true; at least one occurrence has a non-empty pre-format activity currency.
- `N`, unclassified controls: every Annex activity occurrence in both formats supplied with `IsZeroPricedHoldingReduction` equal to false, including a tiny positive unit-price control that displays as `0.00`.
...
- **SC-006**: For 100% of `Z`, `Original Activity Currency` is blank and `Calculation Currency` remains present. For 100% of `N`, the existing original activity currency remains unchanged. Both populations are non-empty in each format.
...
- **SC-011**: Deterministic package, contract, and integration owners verify that the Markdown Annex and PDF Annex contain all 6,666 expected Currency Conversion Audit rows and all three expected entries per row, that Markdown retains controlled entry boundaries, and that the PDF conversion audit occupies at least two pages with complete entries, repeated headers, continuation context, and no clipping or omission. The isolated performance suite does not duplicate these document-contract assertions.
- **SC-012**: Every successful selected-format result, including opener-warning success, identifies the saved files as cleartext financial-data exports, reports every saved path, and tells the user to delete all listed files to remove the exported data. Leaving the result flow retains no report content, path state, history, reopen catalog, or automatic re-ingestion input.
...
- The `IsZeroPricedHoldingReduction` classification and sync-admission semantics are inherited unchanged from Feature 003 FR-017 and Feature 005 FR-029/FR-029a; this feature neither recomputes nor tests those upstream semantics.
...
- Existing report output files remain explicit local user-requested exports outside application-managed persistence only under EXP-001 and FR-028.
