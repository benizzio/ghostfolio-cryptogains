...
3. **Given** a Detailed Per-Asset Audit Report row carries the inherited zero-priced holding-reduction classification from an all-missing, mixed missing-and-zero, or explicit-zero source monetary shape, **When** Annex 1 is generated, **Then** its `Original Activity Currency` cell is blank while its `Calculation Currency` and all other applicable audit evidence remain visible; missing monetary fields remain blank rather than becoming `0.00`.
...
- **FR-013**: Source-price predicates MUST be evaluated from normalized exact source data before conversion and visible formatting. At the report-calculation input boundary, a `zero-priced holding reduction` is the inherited compatibility classification for a normalized `SELL` whose positive quantity reduces holdings without making them negative, whose trimmed explanatory comment is non-empty, and whose every present source monetary field across the order, asset-profile, and base tiers is finite and numerically zero. No source monetary field is required to be present: all-missing, mixed missing-and-zero, explicit-zero-unit-price, and all-explicit-zero source shapes qualify. Numeric zero includes any exponent, trailing-zero scale, or negative-zero sign. Missing remains missing and MUST NOT be converted into zero-valued model or visible output. This report-level compatibility rule does not broaden inherited sync admission; an all-monetary-values-missing upstream activity remains subject to existing amount-resolution validation. In a Detailed Per-Asset Audit Report row carrying the exact pre-format classification, the system MUST leave `Original Activity Currency` blank and MUST NOT infer the classification from a displayed price, activity label, rounded amount, or currency value.
...
- **FR-015**: An `applicable non-zero source price` exists when the inherited priced-activity selector chooses one complete pre-conversion monetary tier in `order -> asset_profile -> base` priority. A complete tier has a non-empty source currency, a present finite fee including exact zero, a finite gross value that is present or derived as quantity multiplied by same-tier unit price, and a finite unit price that is present or derived as same-tier gross value divided by quantity. The selector skips tiers without an explicit currency or monetary value, continues past a higher-priority tier that lacks any complete-tier requirement including fee, never combines values from different tiers, and selects the first complete tier. The chosen unit price MUST be numerically greater than zero. Such a row MUST retain that selected source activity currency as `Original Activity Currency`, including when the positive unit price is small enough to display as `0.00`. A row satisfying neither FR-013 nor this predicate retains its inherited original-currency presentation; this feature suppresses no other row.
...
- `boolean/{true,false}`;
- `currency/{zero-priced-all-missing,zero-priced-explicit-zero,zero-priced-mixed-zero-missing,nonzero,tiny-positive,derived-positive,zero-with-nonzero-source-field}`; and
...
- `Z`, zero-priced controls: every Annex activity occurrence in both formats satisfying FR-013; all-missing, mixed missing-and-zero, and explicit-zero source shapes each occur, missing monetary fields remain absent, and at least one occurrence has a non-empty pre-format activity currency.
...
- The inherited report-calculation compatibility classification accepts zero-priced holding reductions whose source monetary fields are all missing or are any mixture of missing and exact zero; it does not manufacture zero values or broaden sync admission.
