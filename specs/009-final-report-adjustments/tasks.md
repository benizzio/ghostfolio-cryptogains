...
- [X] T039 [P] Update the isolated scale scenario to assert the exact 10,000-activity/two-asset/three-currency workload composition, separate sub-two-minute timers, successful non-empty outputs, opener invocation, and recorded environment data in `tests/performance/helpers_test.go` and `tests/performance/report_performance_flow_test.go`; keep deterministic conversion and PDF-content assertions with their package, contract, and integration owners
...
- [X] T046 Run `make test-performance` for `tests/performance/`, verify each selected-format generation is strictly under two minutes with the retained workload, output, opener, and environment evidence, keep performance evidence out of `dist/coverage.out`, and require or cite the authoritative successful `test-performance / run` Ubuntu check unless the local environment records equivalent runner conditions; deterministic SC-011 pagination/content assertions remain outside this suite
