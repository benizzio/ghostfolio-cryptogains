...
**Bugfix**: 2026-05-15 — [BUG-002] Corrected same-asset ordering to treat Ghostfolio `date` time-of-day as non-authoritative and prefer `BUY` before `SELL` before `source_id`.
**Bugfix**: 2026-05-17 — [BUG-003] Added currency-aware activity-storage requirements from upstream Ghostfolio source review.
**Bugfix**: 2026-05-18 — [BUG-004] Updated Ghostfolio currency-tier dependency assumptions for nullable and optional upstream fields.

**Note**: This template is filled in by the `/speckit.plan` command. See `.specify/templates/plan-template.md` for the execution workflow.

## Summary

Extend the existing Go Bubble Tea sync-validation slice from `specs/002-sync-data-validation/` into a full-history secure-storage slice. The application keeps `setup.json` as the only startup-readable bootstrap state, replaces the one-page activities probe with anonymous auth, authenticated user retrieval, and full activity pagination, normalizes and validates reporting-ready `BUY` and `SELL` activity data using the technical rules established in `specs/001-ghostfolio-gains-reporting/`, and persists successful results only as token-derived encrypted local snapshots. Monetary inputs must remain explicitly tied to their original order-currency, asset-profile-currency, or base-currency context instead of being collapsed under one record-level currency label. `Order.currency` can be `null`, `SymbolProfile.currency` can be omitted, and sync base-currency context comes from authenticated `GET /api/v1/user` `settings.baseCurrency`, which is also optional, so currency-context validation fails only when preserved monetary data remains uninformed across all three independent tiers. Successful sync now ends after protected storage is updated and confirmed. Eligible synced-data failures can also produce local diagnostic reports, with explicit user opt-in outside explicit development mode and automatic generation inside it. Reporting, cached-data browsing, and gains-or-losses calculation remain out of scope.
...
**Constraints**: Bootstrap setup remains the only state readable before token entry; Ghostfolio token and JWT are runtime-only; no raw Ghostfolio payload persistence; support only `BUY` and `SELL`; normalized `BUY` records require non-zero unit price; zero-priced `SELL` records require an explanatory comment and are stored as non-taxable holding reductions; available report years are derived from each source timestamp's own offset and calendar date; Ghostfolio `/api/v1/activities` `date` time-of-day precision is treated as non-authoritative for same-asset ordering, so the original timestamp is preserved for storage and year derivation while same-asset ordering uses source calendar date, then `activity_type` with `BUY` before `SELL`, then `source_id`; deterministic ordering is mandatory; unlock attempts are limited to selected-server snapshot candidates; failed or incompatible replacements must preserve the existing readable snapshot; eligible synced-data failures offer redacted diagnostic-report generation outside explicit development mode and generate it automatically inside explicit development mode; no reporting, preview, or cached-data browsing is exposed in this slice; every stored monetary value must stay tied to explicit order-currency, asset-profile-currency, or base-currency context, but the public Ghostfolio contract can leave `Order.currency`, `SymbolProfile.currency`, or authenticated `settings.baseCurrency` uninformed on a given sync, so those three currency-definition tiers must be treated as independent and currency-context validation must fail only when preserved monetary data remains uninformed across all three tiers  
...
- [x] Precision: Domain parsing and persistence use exact decimal values only. Ghostfolio numeric inputs are parsed as JSON numbers and converted immediately into canonical decimal strings and `apd.Decimal` values. No floating-point values are used in normalized activity storage or validation logic, and this slice still performs no currency conversion or report-boundary rounding. The normalized model must also keep every currency-denominated stored value paired with explicit currency identity instead of inferring one record-level currency.
...
- [x] Dependencies: The only new runtime dependencies planned beyond the existing TUI stack are `github.com/cockroachdb/apd/v3` for exact decimals and `golang.org/x/crypto/argon2` for token-derived key derivation. `research.md` records need, maintenance, freshness, recent activity, and risk for those dependencies and keeps SQL, PDF, and other unnecessary libraries out of this slice. Tasks must preserve traceable research evidence before implementation completes.
- [x] External APIs: Ghostfolio integration remains necessary. The plan documents the observed `POST /api/v1/auth/anonymous`, authenticated `GET /api/v1/user`, and paginated `GET /api/v1/activities` contract, bearer-JWT model, pagination rules, minimum required fields, observed `date` precision limits, contract-drift failure modes, and the rule that the client validates compatibility at runtime instead of assuming a permanently stable upstream API. Current upstream source review confirms the response shape is `Activity extends Order` plus `feeInAssetProfileCurrency`, `feeInBaseCurrency`, `unitPriceInAssetProfileCurrency`, `value`, and `valueInBaseCurrency`; ~~`Order.currency` and `EnhancedSymbolProfile.currency` are present in the public types~~ `Order.currency` is nullable in upstream persistence, `EnhancedSymbolProfile.currency` is optional in the public type, and sync base-currency context comes from authenticated user settings. The design must preserve informed tiers when available and fail currency-context validation only when preserved monetary data remains uninformed across all three independent tiers. Tasks must preserve traceable contract-review evidence before implementation completes.
...
├── checklists/
│   ├── requirements.md
│   └── code-standard-drift-remediation.md
├── quickstart.md
├── contracts/
│   ├── ghostfolio-sync.md
│   └── tui-workflows.md
...
- Reuse the existing `POST /api/v1/auth/anonymous` boundary from `002`, add authenticated `GET /api/v1/user` retrieval for sync base-currency context, and replace the validation-only `take=1` probe with full authenticated pagination against `GET /api/v1/activities`.
...
- Replace `ActivitiesProbeResponse` and `ActivityProbeEntry` with full DTOs and mappers that preserve the fields needed by `001`: source identifier, timestamp, activity type, asset identity, quantity, unit price, gross value, fee amount, explanatory comment, and available source-scope data. From the current upstream Ghostfolio source, that minimum set includes nullable `Order.currency`, `Order.fee`, `Order.unitPrice`, `value`, `feeInAssetProfileCurrency`, `feeInBaseCurrency`, `unitPriceInAssetProfileCurrency`, `valueInBaseCurrency`, required `SymbolProfile`, and optional `SymbolProfile.currency` in addition to identity and timestamp fields; authenticated user retrieval separately contributes optional `settings.baseCurrency` for base-currency context.
...
- Keep currency-aware monetary groupings in the normalized model and stored snapshot: original-order-currency `unitPrice`, `value`, and `fee` stay tied to `currency` when informed; asset-profile-currency derived amounts stay tied to `SymbolProfile.currency` when informed; base-currency derived amounts stay tied to authenticated-user `settings.baseCurrency` when informed. Treat those three currency-definition tiers as independent, and fail currency-context validation only when preserved monetary data remains uninformed across all three tiers.
- The BUG-004 nullable-tier rule does not create a separate cross-tier contradiction class. A single or two absent tiers are acceptable.
- Normalize the complete retrieved history before any persistence step:
  ...
  - preserve the original `occurred_at` value in stored records even when same-asset replay or defensibility uses the source calendar date ordering rule below
...
- Use deterministic ordering for same-asset events that share the same ~~instant~~ source calendar date during replay and defensibility checks: preserve the original `occurred_at` value for storage and year derivation, but treat Ghostfolio `date` time-of-day as non-authoritative for ordering and break ties by `activity_type` with `BUY` before `SELL`, then `source_id`. Reject the sync if the source calendar date cannot be read from `occurred_at` or if the remaining same-asset same-day records still cannot be uniquely ordered by `source_id` after exact-duplicate removal.
...
- Reject normalized history as non-defensible when required reporting fields remain missing or contradictory after normalization, except amounts whose own explicit stored currency field is absent after normalization, currency-context cases where preserved monetary data remains uninformed across all three independent tiers, when replaying an asset timeline would drive its running quantity below zero, or when deterministic same-asset ordering remains ambiguous after ~~`occurred_at` then `source_id`~~ source calendar date, `activity_type`, and `source_id` tie-breaking.
- Preserve Ghostfolio `quantity`, `unitPrice`, and `value` exactly as returned. Do not add a validator rule that rejects a row solely because `value` does not equal `quantity * unitPrice` exactly; upstream production data can carry precision or truncation differences while still remaining usable current-slice basis input.
- Preserve source-scope details when present, but record scope reliability rather than making any reporting decision in this slice. `reliable` means at least one stored asset timeline carries a stable non-empty source-scope identifier and scope kind without contradictions, and no stored timeline is partial. `partial` means any stored asset timeline has scope data for only part of that timeline or conflicts across records. `unavailable` means no usable source-scope data is present. Missing or unreliable scope data does not fail sync by itself, but later reporting must broaden to asset-level scope unless reliability is `reliable`.
...
- Treat the currency-aware stored-activity redesign as an `activity_model_version` change. Compatibility tests and fixtures must prove that older snapshots are rejected or handled according to the supported-version rules until an explicit migration exists.
...
- Add automated checks that same-asset same-day Ghostfolio records with arbitrary time-of-day values are normalized using source calendar date, `activity_type` with `BUY` before `SELL`, then `source_id`, while preserving the original timestamp for storage and year derivation.
- Add automated checks that mixed-currency and nullable-contract activities preserve explicit order-currency, asset-profile-currency, and base-currency identity without collapsing monetary fields under one record-level currency label; `Order.currency = null`, missing `SymbolProfile.currency`, or missing authenticated `settings.baseCurrency` alone do not fail sync when other tiers remain informed; authenticated user retrieval must propagate present and missing `settings.baseCurrency` through mapping and validation; and currency-context failure occurs only when all three independent tiers are uninformed for the preserved monetary data.
...
- Mixed-currency activities add one validation edge case: a single source record can carry legitimate monetary values in different currencies, and Ghostfolio can leave one currency-definition tier uninformed, so the stored model must keep each amount paired with its own informed currency identity while rejecting only all-tier-uninformed.
