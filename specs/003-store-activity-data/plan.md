...
- Reject normalized history as non-defensible when required reporting fields remain missing or contradictory after normalization, when replaying an asset timeline would drive its running quantity below zero, or when deterministic same-asset ordering remains ambiguous after `occurred_at` then `source_id` tie-breaking.
