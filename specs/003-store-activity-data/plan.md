...
- Keep currency-aware monetary groupings in the normalized model and stored snapshot: original-order-currency `unitPrice`, `value`, and `fee` stay tied to `currency`; asset-profile-currency derived amounts stay tied to `SymbolProfile.currency`; base-currency derived amounts stay tied to the sync base-currency context. Reject or fail validation when required currency context is missing or contradictory.
...
- Add automated checks that mixed-currency activities preserve explicit order-currency, asset-profile-currency, and base-currency identity without collapsing monetary fields under one record-level currency label, and that incomplete or contradictory currency context is rejected before persistence.
...
- Mixed-currency activities add one validation edge case: a single source record can carry legitimate monetary values in different currencies, so the stored model must keep each amount paired with its own currency identity while still rejecting incomplete context.
