...
- Replace `ActivitiesProbeResponse` and `ActivityProbeEntry` with full DTOs and mappers that preserve the fields needed by `001`: source identifier, timestamp, activity type, asset identity, quantity, unit price, gross value, fee amount, explanatory comment, and available source-scope data. From the current upstream Ghostfolio source, that minimum set includes `Order.currency`, `Order.fee`, `Order.unitPrice`, `value`, `feeInAssetProfileCurrency`, `feeInBaseCurrency`, `unitPriceInAssetProfileCurrency`, `valueInBaseCurrency`, and `SymbolProfile.currency` in addition to identity and timestamp fields. The boundary documentation must also distinguish mandatory, optional, and nullable inputs for this slice because live Ghostfolio data can return `Order.currency = null`.
...
- Keep currency-aware monetary groupings in the normalized model and stored snapshot: original-order-currency `unitPrice`, `value`, and `fee` stay tied to `currency` when Ghostfolio provides it; if Ghostfolio returns `Order.currency = null`, preserve the related order-currency amounts and null currency unchanged. Asset-profile-currency derived amounts stay tied to `SymbolProfile.currency`; base-currency derived amounts stay tied to the sync base-currency context. Reject or fail validation only when required currency context is contradictory or missing from fields that Ghostfolio or authenticated user settings are expected to provide.
...
- Reject normalized history as non-defensible when required reporting fields remain missing or contradictory after normalization, when replaying an asset timeline would drive its running quantity below zero, or when deterministic same-asset ordering remains ambiguous after ~~`occurred_at` then `source_id`~~ source calendar date, `activity_type`, and `source_id` tie-breaking. Nullable upstream fields allowed by Ghostfolio, including `Order.currency = null`, are preserved inputs rather than automatic sync failures on their own.
...
- Add automated checks that mixed-currency activities preserve explicit order-currency, asset-profile-currency, and base-currency identity without collapsing monetary fields under one record-level currency label, that Ghostfolio-returned nullable `Order.currency` values are stored without sync failure, and that only contradictory or dependency-impossible currency context is rejected before persistence.
...
- Mixed-currency activities add one validation edge case: a single source record can carry legitimate monetary values in different currencies, and live Ghostfolio data can also carry order-scoped monetary values with `Order.currency = null`, so the stored model must keep provided currency identity where available, preserve nullable upstream omissions without relabeling, and still reject contradictory or dependency-impossible context.
