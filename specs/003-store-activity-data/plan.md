...
- Use deterministic ordering for same-asset events that share the same ~~instant~~ source calendar date: preserve the original `occurred_at` value for storage and year derivation, but treat Ghostfolio `date` time-of-day as non-authoritative for ordering and break ties by `activity_type` with `BUY` before `SELL`, then `source_id`. Reject the sync if stable ordering cannot be established.
