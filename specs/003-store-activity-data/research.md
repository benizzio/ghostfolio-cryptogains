...
Decision: Expand the current Ghostfolio boundary from a one-page communication probe into a full authenticated retrieval flow using `POST /api/v1/auth/anonymous`, authenticated `GET /api/v1/user`, and paginated `GET /api/v1/activities` requests until the full available history has been collected.

Rationale: `specs/003-store-activity-data/spec.md` turns `Sync Data` into full-history retrieval and protected persistence. `specs/001-ghostfolio-gains-reporting/contracts/ghostfolio-sync.md` already documents the observed `api/v1` auth and activities contract, including pagination rules and minimum required fields. The refreshed contract review on 2026-05-17 confirms Ghostfolio release `3.3.0` published on 2026-05-14 and current upstream controller code in `apps/api/src/app/auth/auth.controller.ts`, `apps/api/src/app/user/user.controller.ts`, `apps/api/src/app/activities/activities.controller.ts`, and `apps/api/src/app/activities/activities.service.ts`, plus public interfaces in `libs/common/src/lib/interfaces/user.interface.ts`, `libs/common/src/lib/interfaces/user-settings.interface.ts`, and `libs/common/src/lib/interfaces/activities.interface.ts`. That source still exposes `@Post('anonymous')`, returns `{ authToken }`, exposes authenticated `GET /user`, and exposes `GET /activities` with `skip`, `take`, `sortColumn`, and `sortDirection`, returning `{ activities, count }` while the service keeps date ordering and appends `id` as a deterministic tie-break when sorting. The verified public user contract carries `settings.baseCurrency`, while the verified public `Activity` contract exposes `currency`, `fee`, `unitPrice`, `value`, `feeInAssetProfileCurrency`, `feeInBaseCurrency`, `unitPriceInAssetProfileCurrency`, `valueInBaseCurrency`, and `SymbolProfile.currency`, but does not establish a required per-activity `baseCurrency` field. The current `002` client and runtime service were intentionally transitional, and the `002` plan explicitly says their probe DTOs must be removed or evolved when real sync is introduced.

Selected retrieval rules:

- keep the existing anonymous-auth request model and runtime-only JWT handling
- fetch authenticated user settings after auth and derive the sync base-currency context from `settings.baseCurrency`
