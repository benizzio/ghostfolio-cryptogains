...
- Dependency and API evidence: `research.md` refreshes due diligence for `github.com/cockroachdb/apd/v3` and `golang.org/x/crypto/argon2`; `research.md` and `contracts/ghostfolio-sync.md` refresh the Ghostfolio `3.3.0` auth, authenticated `GET /user`, and paginated activities contract review, including the non-authoritative `date` time-of-day note for same-asset ordering, the verified `User.settings.baseCurrency` source for sync base-currency context, the verified mixed-currency activity fields `currency`, `fee`, `unitPrice`, `value`, `feeInAssetProfileCurrency`, `feeInBaseCurrency`, `unitPriceInAssetProfileCurrency`, `valueInBaseCurrency`, and `SymbolProfile.currency`, and the BUG-004 nullable-tier propagation rerun that now covers `Order.currency = null`, missing `SymbolProfile.currency`, and omitted `settings.baseCurrency`.
...
- Final BUG-004 verification rerun: `make test`, `make coverage`, and `GHOSTFOLIO_CRYPTOGAINS_RUN_PERFORMANCE=1 go test ./tests/integration -run TestSyncPerformanceFlowLargeHistoryFixture -count=1 -v` all passed on 2026-05-18, regenerated `dist/coverage/coverage.out` plus `dist/coverage/coverage.xml`, and completed the documented `SC-006` performance path in `300.22878ms` for `10000` activities across `6` available report years.
