...
- [ ] T078 [P] [US2] Add reusable report fixtures for priced `BUY` rows where order financial values exist but `order_currency` is absent and asset or base tiers can still supply the activity input in `tests/testutil/report_fixtures.go`
...
- [ ] T079 [P] [US2] Add integration regression coverage that a priced `BUY` with a currencyless order tier proceeds with the first later explicit-currency tier that can supply or exactly derive same-tier values instead of failing on the order tier in `tests/integration/report_generation_flow_test.go`
