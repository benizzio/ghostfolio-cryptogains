...
- [ ] T086 [P] Add unit coverage for repeating gross-value-only unit-price derivation acceptance at the Ghostfolio response-validation and synced-history amount-resolution boundaries in `internal/ghostfolio/validator/response_validator_more_internal_test.go` and `internal/sync/model/activity_amount_resolution_internal_test.go`
- [ ] T087 Replace pre-report exact unit-price derivation gates in `internal/ghostfolio/validator/response_validator.go` and `internal/sync/model/activity_amount_resolution.go` so gross-value-only rows requiring repeating `gross_value / quantity` are not rejected when same-tier inputs are otherwise valid
...
- [ ] T089 [P] [US2] Add integration coverage proving a gross-value-only activity with repeating `gross_value / quantity` passes sync-boundary validation and remains reportable under the selected same-tier context in `tests/integration/report_generation_flow_test.go` or a sync-boundary integration suite
...
- [ ] T090 [US2] Extend report model types and constructors in `internal/report/model/report.go` so priced liquidation calculations carry fragment-level matches with `matched_proceeds` and `matched_gain_or_loss` through to Markdown rendering inputs
