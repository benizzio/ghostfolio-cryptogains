...
- [X] T032 [P] [US2] ⚠️ Reopened Add unit coverage for single-activity currency context priority, skipping tiers without explicit currency before completeness validation, explicit zero fee, missing fee, positive priced quantity, exact same-tier gross-value derivation by multiplication, exact unit-price derivation, multiplication preference over division-based fallbacks, no cross-tier mixing, and explained zero-priced rows with preserved explicit zero-valued source fields in `tests/unit/report_activity_input_test.go` (reopened — BUG-001, BUG-007)
...
- [X] T033 [P] [US2] ⚠️ Reopened Implement single-activity currency context selection, selected-currency tracking, and calculation-input validation in `internal/report/calculate/activity_input.go` so tiers without explicit currency are skipped, same-tier exact gross-value derivation by multiplication is preferred before division-based fallbacks, and explained zero-priced holding reductions can preserve explicit zero-valued source fields without becoming priced inputs or requiring activity currency context (reopened — BUG-001, BUG-007)
...
- [X] T037 [US2] ⚠️ Reopened Implement report calculation engine for asset timelines, source-year cutoff, opening and closing basis, inclusion rules, same-date reopening behavior, reference entries, summary entries, yearly net total, shared report-calculation-currency enforcement, and lower-priority explicit-currency context acceptance after currencyless higher-priority tiers in `internal/report/calculate/calculator.go` (reopened — BUG-007)
...
- [X] T039 [US2] ⚠️ Reopened Implement non-secret report calculation error taxonomy with offending activity source ID and display label references in `internal/report/model/errors.go` and `internal/report/calculate/calculator.go`, preserve the original offending persisted `ActivityRecord` for diagnostics without substituting derived report inputs, preserve an ordered wrapped-cause chain for diagnostics instead of flattening nested failures into `failure_detail`, and report terminal incomplete-context failure only after explicit-currency tiers are exhausted or a selected explicit-currency tier cannot derive required values safely (reopened — BUG-006, BUG-007, BUG-008)
- [X] T079 [P] [US2] Add integration regression coverage that a priced `BUY` with a currencyless order tier proceeds with the first later explicit-currency tier that can supply or exactly derive same-tier values instead of failing on the order tier in `tests/integration/report_generation_flow_test.go`
...
- T025 through T032, T065, T074, T076, T078, T079, and T080 can run in parallel for US2; T033 through T036, T040 through T043, T066, and T075 can then run in parallel before T037, T038, T044, T045, T047, and T048.
...
- T058, T059, T060, T061, T077, and T081 can run in parallel after story implementation; T062, T063, and T064 run after documentation and performance fixtures are in place.
