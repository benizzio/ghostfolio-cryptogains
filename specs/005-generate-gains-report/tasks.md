...
- [X] T062 ⚠️ Reopened Run `make test` and `make coverage`, then verify report-feature coverage artifacts in `dist/coverage/coverage.out` and `dist/coverage/coverage.xml`, including the report-diagnostics and shared-sync-diagnostics explicit-`null` regression paths and the currencyless-order-tier activity-input regression paths (reopened — BUG-006, BUG-007)
...
- T025 through T032, T065, T074, T076, T078, and T079 can run in parallel for US2; T033 through T036, T040 through T043, T066, and T075 can then run in parallel before T037, T038, T044, T045, T047, and T048.
