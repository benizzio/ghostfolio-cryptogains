...
- [ ] T011 [P] ⚠️ Reopened Reuse `internal/support/decimal` for canonical formatting, then add report-specific helpers for the shared 16-decimal internal calculation precision, including round-half-up division and allocation handling, plus multiplication, zero checks, and comparisons in `internal/report/calculate/decimal_math.go` (reopened — BUG-009)
- [ ] T082 [P] Define one shared report-calculation decimal policy of 16 places with round-half-up rounding and thread it through report helpers and calculator entry points in `internal/report/calculate/decimal_math.go` and `internal/report/calculate/calculator.go`
...
- [ ] T035 [P] [US2] ⚠️ Reopened Implement FIFO, LIFO, and HIFO lot basis state with exact arithmetic, deterministic lot ordering, and shared 16-decimal internal division and allocation precision in `internal/report/basis/lot_methods.go` (reopened — BUG-009)
- [ ] T036 [P] [US2] ⚠️ Reopened Implement Average Cost Basis pool state, shared 16-decimal internal calculation precision, and zero-quantity pool reset in `internal/report/basis/average_cost.go` (reopened — BUG-009)
- [ ] T037 [US2] ⚠️ Reopened Implement report calculation engine for asset timelines, source-year cutoff, opening and closing basis, inclusion rules, same-date reopening behavior, reference entries, summary entries, yearly net total, shared 16-decimal internal calculation precision for basis, proceeds, and derived values, shared report-calculation-currency enforcement, and lower-priority explicit-currency context acceptance after currencyless higher-priority tiers in `internal/report/calculate/calculator.go` (reopened — BUG-007, BUG-009)
...
- [ ] T051 [P] [US3] ⚠️ Reopened Add unit coverage for HIFO unit-cost tie-breaking, rounded non-terminating division at the shared 16-decimal internal calculation precision, scope-local reliable scope resolution, fallback activation, fallback carry-forward until zero, same-scope reset after reacquisition, and independent other-scope state in `tests/unit/report_basis_methods_test.go` (reopened — BUG-009)
