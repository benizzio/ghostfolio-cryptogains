...
**Bugfix**: 2026-05-23 — [BUG-008] Updated from bugfix patch

**Bugfix**: 2026-05-23 — [BUG-009] Updated from bugfix patch

**Bugfix**: 2026-05-24 — [BUG-009-1] Updated from bugfix patch
...
- [X] T011 [P] ⚠️ Reopened Reuse `internal/support/decimal` for canonical formatting, then add report-specific helpers for the shared 16-decimal internal calculation precision, including round-half-up division and allocation handling, plus multiplication, zero checks, and comparisons in `internal/report/calculate/decimal_math.go` (reopened — BUG-009)
- [X] T082 [P] Define one shared report-calculation decimal policy of 16 places with round-half-up rounding and thread it through report helpers and calculator entry points in `internal/report/calculate/decimal_math.go` and `internal/report/calculate/calculator.go`
- [X] T086 [P] Add unit coverage for repeating gross-value-only unit-price derivation acceptance at the Ghostfolio response-validation and synced-history amount-resolution boundaries in `internal/ghostfolio/validator/response_validator_more_internal_test.go` and `internal/sync/model/activity_amount_resolution_internal_test.go`
- [X] T087 Replace pre-report exact unit-price derivation gates in `internal/ghostfolio/validator/response_validator.go` and `internal/sync/model/activity_amount_resolution.go` so gross-value-only rows requiring repeating `gross_value / quantity` are not rejected when same-tier inputs are otherwise valid
...
- [X] T028 [P] [US2] ⚠️ Reopened Add integration coverage for empty-main-section reports with `NOT APPLICABLE` calculation currency, incomplete monetary-context failure only after all explicit-currency tiers are unusable, Documents unavailable failure, partial-file cleanup, app-managed storage leakage checks, report-failure diagnostics file creation, explicit-development-mode behavior, activity-specific diagnostics that carry the original persisted activity record with explicit `null` values and no derived substitute fields, and wrapped-failure diagnostics that preserve both `failure_detail` and ordered `failure_cause_chain` for calculation and output-preparation failures in `tests/integration/report_failure_flow_test.go` (reopened — BUG-006, BUG-007, BUG-008)
- [X] T084 [P] [US2] Add integration regression coverage for deterministic report histories that require repeating-decimal calculations yet remain valid under the shared 16-decimal internal calculation precision in `tests/integration/report_generation_flow_test.go` and `tests/integration/report_cost_basis_methods_flow_test.go`
...
- [X] T032 [P] [US2] ⚠️ Reopened Add unit coverage for single-activity currency context priority, skipping tiers without explicit currency before completeness validation, explicit zero fee, missing fee, positive priced quantity, same-tier gross-value derivation by multiplication, same-tier unit-price derivation at the shared 16-decimal internal calculation precision, multiplication preference over division-based fallbacks, no cross-tier mixing, and explained zero-priced rows with preserved explicit zero-valued source fields in `tests/unit/report_activity_input_test.go` (reopened — BUG-001, BUG-007, BUG-009)
- [X] T083 [P] [US2] ⚠️ Reopened Add unit coverage for successful rounded non-terminating divisions in average-cost unit cost, lot-fragment basis allocation, fragment-level proportional proceeds allocation, and same-tier derivations at the shared 16-decimal internal calculation precision, including a differentiating rounded-intermediate allocation case such as disposing `2` units from `1` basis over `3` total quantity, in `tests/unit/report_calculation_test.go`, `tests/unit/report_activity_input_test.go`, and `tests/unit/report_basis_methods_test.go` (reopened — BUG-009-1)
- [X] T088 [P] [US2] Add report regression coverage for multi-fragment priced liquidations that retain `BasisMatch` entries with `matched_proceeds` and `matched_gain_or_loss`, including repeating proportional-proceeds math, in `tests/unit/report_calculation_test.go` and `tests/integration/report_cost_basis_methods_flow_test.go`
- [X] T089 [P] [US2] Add integration coverage proving a gross-value-only activity with repeating `gross_value / quantity` passes sync-boundary validation and remains reportable under the selected same-tier context in `tests/integration/report_generation_flow_test.go` or a sync-boundary integration suite
- [X] T078 [P] [US2] Add reusable report fixtures for priced `BUY` rows where order financial values exist but `order_currency` is absent and asset or base tiers can still supply the activity input in `tests/testutil/report_fixtures.go`
...
- [X] T074 [P] [US2] ⚠️ Reopened Add shared diagnostic-report unit coverage that nullable source fields serialize as explicit `null`, activity-specific report failures use original persisted `ActivityRecord` context, and wrapped failures serialize deterministic outer-to-inner `failure_cause_chain` entries in `internal/app/runtime/runtime_more_internal_test.go` and `internal/sync/model/diagnostic_context_internal_test.go` (reopened — BUG-008)
- [X] T076 [P] [US2] Add integration regression coverage that existing synced-data diagnostics also serialize absent source fields as explicit `null` instead of omitting them in `tests/integration/sync_diagnostic_report_flow_test.go` and `tests/integration/diagnostic_redaction_test.go`
- [X] T080 [P] [US2] Add nested-cause diagnostics security regression coverage proving secret-bearing wrapped causes are redacted or suppressed before they enter `failure_cause_chain` in `tests/integration/diagnostic_redaction_test.go` and `tests/integration/report_failure_flow_test.go`

### Implementation for User Story 2

- [X] T033 [P] [US2] ⚠️ Reopened Implement single-activity currency context selection, selected-currency tracking, and calculation-input validation in `internal/report/calculate/activity_input.go` so tiers without explicit currency are skipped, same-tier gross-value derivation by multiplication is preferred before division-based fallbacks, same-tier unit-price division uses the shared 16-decimal internal calculation precision, and explained zero-priced holding reductions can preserve explicit zero-valued source fields without becoming priced inputs or requiring activity currency context (reopened — BUG-001, BUG-007, BUG-009)
...
- [X] T090 [US2] Extend report model types and constructors in `internal/report/model/report.go` so priced liquidation calculations carry fragment-level matches with `matched_proceeds` and `matched_gain_or_loss` through to Markdown rendering inputs
- [X] T035 [P] [US2] ⚠️ Reopened Implement FIFO, LIFO, and HIFO lot basis state with exact arithmetic, deterministic lot ordering, and shared 16-decimal internal division and allocation precision in `internal/report/basis/lot_methods.go` (reopened — BUG-009)
- [X] T036 [P] [US2] ⚠️ Reopened Implement Average Cost Basis pool state, shared 16-decimal internal calculation precision, and zero-quantity pool reset in `internal/report/basis/average_cost.go` (reopened — BUG-009)
- [X] T037 [US2] ⚠️ Reopened Implement report calculation engine for asset timelines, source-year cutoff, opening and closing basis, inclusion rules, same-date reopening behavior, reference entries, summary entries, yearly net total, shared 16-decimal internal calculation precision for basis, proceeds, and derived values, shared report-calculation-currency enforcement, and lower-priority explicit-currency context acceptance after currencyless higher-priority tiers in `internal/report/calculate/calculator.go` (reopened — BUG-007, BUG-009)
- [X] T038 [US2] ⚠️ Reopened Implement priced liquidation proceeds, fragment-level proportional proceeds allocation, matched gain or loss, explained zero-priced holding reductions, and basis removal details in `internal/report/calculate/calculator.go`, preserving explicit zero-valued source fields without creating proceeds, gains, losses, or priced-liquidation behavior (reopened — BUG-001, BUG-009-1)
- [X] T039 [US2] ⚠️ Reopened Implement non-secret report calculation error taxonomy with offending activity source ID and display label references in `internal/report/model/errors.go` and `internal/report/calculate/calculator.go`, preserve the original offending persisted `ActivityRecord` for diagnostics without substituting derived report inputs, preserve an ordered wrapped-cause chain for diagnostics instead of flattening nested failures into `failure_detail`, and treat non-terminating divisions resolved under the shared 16-decimal internal calculation precision as valid calculations rather than terminal errors while reporting incomplete-context failure only after explicit-currency tiers are exhausted or a selected explicit-currency tier cannot derive required values safely (reopened — BUG-006, BUG-007, BUG-008, BUG-009)
- [X] T079 [P] [US2] Add integration regression coverage that a priced `BUY` with a currencyless order tier proceeds with the first later explicit-currency tier that can supply or derive same-tier values instead of failing on the order tier in `tests/integration/report_generation_flow_test.go`
...
- [X] T075 [P] [US2] ⚠️ Reopened Extend the shared diagnostics context and JSON writer so synced-data and report-failure diagnostics preserve nullable source fields as explicit `null`, can carry report-generation failure categories with original persisted activity records, and serialize ordered `failure_cause_chain` data beside `failure_detail` in `internal/sync/model/diagnostic_context.go`, `internal/app/runtime/diagnostic_report.go`, and `internal/app/runtime/report_types.go` (reopened — BUG-008)
...
- [X] T051 [P] [US3] ⚠️ Reopened Add unit coverage for HIFO unit-cost tie-breaking, rounded non-terminating division at the shared 16-decimal internal calculation precision, rounded-intermediate partial-fragment allocation cases that distinguish the active formula ordering from direct proportional shortcuts, scope-local reliable scope resolution, fallback activation, fallback carry-forward until zero, same-scope reset after reacquisition, and independent other-scope state in `tests/unit/report_basis_methods_test.go` (reopened — BUG-009, BUG-009-1)
...
- [X] T085 [P] Reconcile internal 16-decimal calculation precision versus non-rounded report rendering in `specs/005-generate-gains-report/research.md`, `specs/005-generate-gains-report/data-model.md`, `specs/005-generate-gains-report/quickstart.md`, and `specs/005-generate-gains-report/checklists/calculation.md`
- [X] T091 [P] Reconcile supporting artifacts and diagnostics expectations that still say `exactly derive` or `same-tier exact derivation` after BUG-009 in `specs/005-generate-gains-report/contracts/markdown-report.md`, `specs/005-generate-gains-report/quickstart.md`, `specs/005-generate-gains-report/checklists/requirements.md`, and affected diagnostics tests
- [X] T060 [P] ⚠️ Reopened Add OWASP Top 10, cryptographic-storage boundary, report cleartext output, report-failure diagnostics artifact review, original persisted activity-data inclusion, explicit `null` rendering, nested wrapped-cause redaction or suppression, and dependency/API review evidence in `specs/005-generate-gains-report/checklists/requirements.md` (reopened — BUG-006, BUG-008)
...
- [X] T062 ⚠️ Reopened Run `make test` and `make coverage`, then verify report-feature coverage artifacts in `dist/coverage/coverage.out` and `dist/coverage/coverage.xml`, including the report-diagnostics and shared-sync-diagnostics explicit-`null` regression paths, wrapped-cause `failure_cause_chain` regression paths, the currencyless-order-tier activity-input regression paths, fragment-level matched-proceeds regression paths, sync-boundary repeating unit-price derivation acceptance, and the 16-decimal rounded-division regression paths (reopened — BUG-006, BUG-007, BUG-008, BUG-009, BUG-009-1)
...
- [X] T064 ⚠️ Reopened Inspect generated test artifacts and application-managed storage for cleartext report leakage and report-failure diagnostics artifacts, then document the result in `specs/005-generate-gains-report/checklists/requirements.md` (reopened — BUG-006)
- [X] T077 [P] Reconcile report-failure diagnostics outcome state, failure-result workflow contract, and manual verification steps in `specs/005-generate-gains-report/data-model.md`, `specs/005-generate-gains-report/contracts/tui-workflows.md`, and `specs/005-generate-gains-report/quickstart.md`
- [X] T081 [P] Reconcile diagnostics artifact schema and manual inspection guidance for ordered `failure_cause_chain` entries in `specs/005-generate-gains-report/data-model.md`, `specs/005-generate-gains-report/quickstart.md`, `specs/005-generate-gains-report/checklists/requirements.md`, and `specs/004-source-faithful-diagnostics/spec.md`

---

## Phase 7: Coding Standards Drift Remediation

**Purpose**: Remediate the active coding-standards findings from `specs/005-generate-gains-report/coding-standards-drift-report.md` after the feature implementation and polish phases are complete.

- [X] T092 DRIFT-001 High: Sync Model Owns Validation-Like Amount Resolution by moving transient amount-resolution policy out of `internal/sync/model/activity_amount_resolution.go` into the validation boundary in `internal/sync/validate/activity_amount_resolution.go`, updating `internal/sync/validate/activity_history.go`, and relocating coverage from `internal/sync/model/activity_amount_resolution_internal_test.go` into validation-boundary tests per `specs/005-generate-gains-report/coding-standards-drift-report.md#drift-001-sync-model-owns-validation-like-amount-resolution`; evidence: `internal/sync/model/activity_amount_resolution.go`
- [X] T093 DRIFT-002 Medium: Round-Half-Up Decimal Policy Is Duplicated Across Packages by centralizing the fixed-scale round-half-up quotient and finite-decimal helpers in `internal/support/math/rounding.go` with coverage in `internal/support/math/rounding_internal_test.go`, then replacing duplicate implementations in `internal/report/decimal/policy.go`, `internal/ghostfolio/validator/response_validator.go`, and `internal/sync/model/activity_amount_resolution.go` or its validation-boundary successor per `specs/005-generate-gains-report/coding-standards-drift-report.md#drift-002-round-half-up-decimal-policy-is-duplicated-across-packages`; evidence: `internal/report/decimal/policy.go`, `internal/ghostfolio/validator/response_validator.go`, `internal/sync/model/activity_amount_resolution.go`
- [X] T094 DRIFT-003 Medium: TUI Handlers Exceed The Cognitive Complexity Threshold by splitting `(*Model).handleReportSelectionKey` in `internal/tui/flow/report_flow.go` and `(*Model).updateSyncResult` in `internal/tui/flow/navigation.go` into smaller focus-movement, action-dispatch, and result-routing helpers, with regression coverage updates in `internal/tui/flow/model_internal_test.go` and affected workflow tests per `specs/005-generate-gains-report/coding-standards-drift-report.md#drift-003-tui-handlers-exceed-the-cognitive-complexity-threshold`; evidence: `internal/tui/flow/report_flow.go`, `internal/tui/flow/navigation.go`
- [X] T095 DRIFT-004 Medium: Asset Replay Calculation Exceeds The Cognitive Complexity Threshold by decomposing `calculateAssetGroup` across `internal/report/calculate/calculator.go` and `internal/report/calculate/asset_replay.go`, separating replay, yearly accumulation, and final result construction, and updating `internal/report/calculate/calculator_internal_test.go` plus `tests/unit/report_calculation_test.go` per `specs/005-generate-gains-report/coding-standards-drift-report.md#drift-004-asset-replay-calculation-exceeds-the-cognitive-complexity-threshold`; evidence: `internal/report/calculate/calculator.go`
- [X] T096 DRIFT-005 Medium: Lot Disposal Calculation Exceeds The Cognitive Complexity Threshold by decomposing `(*LotMethodState).Dispose` in `internal/report/basis/lot_methods.go` into focused validation, ordered-lot iteration, proportional-basis allocation, and mutation helpers, and updating `internal/report/basis/lot_methods_internal_test.go` plus `tests/unit/report_basis_methods_test.go` per `specs/005-generate-gains-report/coding-standards-drift-report.md#drift-005-lot-disposal-calculation-exceeds-the-cognitive-complexity-threshold`; evidence: `internal/report/basis/lot_methods.go`
- [X] T097 DRIFT-006 Medium: XDG Documents Parser Exceeds The Cognitive Complexity Threshold by splitting `parseXDGDocumentsDirectory` in `internal/report/output/documents.go` into focused XDG entry extraction and path-resolution helpers, with coverage updates in `internal/report/output/documents_internal_test.go` and `tests/unit/report_output_test.go`, per `specs/005-generate-gains-report/coding-standards-drift-report.md#drift-006-xdg-documents-parser-exceeds-the-cognitive-complexity-threshold`; evidence: `internal/report/output/documents.go`
- [X] T098 DRIFT-007 Medium: Runtime Classifies Output Failures By Message Text by introducing typed output failure categories in `internal/report/output/errors.go`, threading them through `internal/report/output/documents.go` and `internal/report/output/writer.go`, replacing substring matching in `internal/app/runtime/report_service.go`, and updating `internal/app/runtime/report_service_internal_test.go` plus `tests/integration/report_failure_flow_test.go` per `specs/005-generate-gains-report/coding-standards-drift-report.md#drift-007-runtime-classifies-output-failures-by-message-text`; evidence: `internal/app/runtime/report_service.go`
- [X] T099 DRIFT-008 Medium: Report Output Package Exposes Test-Only Hooks In Production Code by removing exported testing hooks from `internal/report/output/seams.go`, moving deterministic failure injection behind package-internal or runtime-owned dependencies in `internal/report/output/writer.go`, and updating `internal/report/output/writer_internal_test.go`, `internal/report/output/documents_internal_test.go`, and `tests/integration/report_failure_flow_test.go` per `specs/005-generate-gains-report/coding-standards-drift-report.md#drift-008-report-output-package-exposes-test-only-hooks-in-production-code`; evidence: `internal/report/output/seams.go`
- [X] T100 DRIFT-009 Medium: Menu Actions Depend On Raw Index Positions by introducing named action identifiers in `internal/tui/flow/menu_actions.go`, wiring them through `internal/tui/flow/model.go`, `internal/tui/flow/report_flow.go`, `internal/tui/flow/navigation.go`, and `internal/tui/flow/sync_flow.go`, and updating `internal/tui/flow/model_internal_test.go` per `specs/005-generate-gains-report/coding-standards-drift-report.md#drift-009-menu-actions-depend-on-raw-index-positions`; evidence: `internal/tui/flow/model.go`, `internal/tui/flow/report_flow.go`, `internal/tui/flow/navigation.go`, `internal/tui/flow/model.go`
- [X] T101 DRIFT-010 Low: AI-Generated Documentation Is Incomplete Or Stale by adding missing AI-authored type and API documentation plus authoring information in `internal/ghostfolio/mapper/activity_mapper.go`, `internal/app/runtime/report_types.go`, and `internal/tui/flow/report_flow.go`, including detailed public usage guidance for `ReportService.Generate`, per `specs/005-generate-gains-report/coding-standards-drift-report.md#drift-010-ai-generated-documentation-is-incomplete-or-stale`; evidence: `internal/ghostfolio/mapper/activity_mapper.go`, `internal/app/runtime/report_types.go`, `internal/tui/flow/report_flow.go`
- [X] T102 DRIFT-011 Medium: UI Copy Centralization Is Partial And Inconsistent by consolidating shared unlock, context-menu, report-selection, report-result, sync-result, and main-menu copy in `internal/tui/component/workflow_copy.go`, replacing inline wording in `internal/tui/flow/model.go`, `internal/tui/screen/sync_reports_screen.go`, `internal/tui/screen/report_screen.go`, `internal/tui/screen/sync_result_screen.go`, and `internal/tui/screen/main_menu_screen.go`, and updating affected screen and flow tests per `specs/005-generate-gains-report/coding-standards-drift-report.md#drift-011-ui-copy-centralization-is-partial-and-inconsistent`; evidence: `internal/tui/component/sync_entry_copy.go`, `internal/tui/screen/sync_entry_screen.go`, `internal/tui/flow/model.go`, `internal/tui/screen/sync_reports_screen.go`, `internal/tui/screen/report_screen.go`, `internal/tui/screen/sync_result_screen.go`, `internal/tui/screen/main_menu_screen.go`
- [X] T103 DRIFT-012 Medium: Generic Decimal Math Helpers Bypass The Shared Support Boundary by creating reusable arithmetic and allocation helpers in `internal/support/math/decimal_ops.go` with coverage in `internal/support/math/decimal_ops_internal_test.go`, replacing report-local generic helpers in `internal/report/basis/lot_methods.go`, `internal/report/calculate/decimal_math.go`, `internal/report/calculate/calculator.go`, and `internal/report/decimal/policy.go`, and updating affected report-package tests per `specs/005-generate-gains-report/coding-standards-drift-report.md#drift-012-generic-decimal-math-helpers-bypass-the-shared-support-boundary`; evidence: `internal/report/basis/lot_methods.go`, `internal/report/calculate/decimal_math.go`, `internal/report/calculate/calculator.go`, `internal/report/decimal/policy.go`
- [X] T104 Run `make test`, `make coverage`, and `go run github.com/uudashr/gocognit/cmd/gocognit@latest -over 15 ./internal/tui/flow ./internal/report/calculate ./internal/report/basis ./internal/report/output` to verify the coding-standards remediation phase from `specs/005-generate-gains-report/coding-standards-drift-report.md`, then inspect `dist/coverage/coverage.out` and `dist/coverage/coverage.xml` for regressions before closing the feature

**Checkpoint**: The active coding-standards drift findings are remediated and re-verified with tests, coverage, and targeted cognitive-complexity checks.
...
- Phase 7 depends on Phase 6 and closes the feature after the existing implementation, documentation, coverage, and performance work is complete.
...
- T004, T006, T008, T009, T010, T011, T067, T082, and T086 can run in parallel after Phase 1; T005, T007, T012, T068, and T087 close after the related model, snapshot, fixture, and boundary-validation decisions exist.
- T013, T014, T015, T069, T071, and T073 can run in parallel for US1 before T016 through T024, T070, and T072.
- T025 through T032, T065, T074, T076, T078, T079, T080, T083, T084, T088, and T089 can run in parallel for US2; T033 through T036, T040 through T043, T066, T075, and T090 can then run in parallel before T037, T038, T044, T045, T047, and T048.
...
- T058, T059, T060, T061, T077, T081, T085, and T091 can run in parallel after story implementation; T062, T063, and T064 run after documentation and performance fixtures are in place.
...
Task: T078 Add currencyless-order-tier BUY fixtures in tests/testutil/report_fixtures.go
Task: T065 Add explicit-zero explained SELL fixtures in tests/testutil/report_fixtures.go
Task: T079 Add lower-priority explicit-currency integration regression coverage in tests/integration/report_generation_flow_test.go
...
5. Finish Phase 7 to remediate the active coding-standards drift before closing the slice.
