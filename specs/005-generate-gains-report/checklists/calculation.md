...
- [ ] CHK006 Is the term `reportable year` defined precisely enough to derive year choices without interpretation drift? [Clarity, Spec FR-005, Spec FR-006]
- [ ] CHK007 Is `first acquisition` unambiguous about whether reopening events, transfers, or zero-priced holding reductions can affect exclusion under `FR-014`? [Ambiguity, Spec FR-014, Spec FR-017]
...
- [ ] CHK025 Does the spec define calculation requirements for same-timestamp activity ordering when HIFO ties or reopened-position decisions depend on deterministic order? [Edge Case, Spec HIFO, Spec FR-017]
- [ ] CHK026 Are requirements explicit about how zero-quantity or zero-fee but priced activities are treated if such records appear in synced data, or is their exclusion only assumed? [Gap, Assumption, Spec FIN-001, Spec INT-001]
- [ ] CHK027 Are cross-year boundary cases addressed for activities exactly at year start or year end, including how opening, in-year, and later activity classification is determined? [Edge Case, Gap, Spec FR-015, Spec FR-018]
...
- [ ] CHK030 Is dependence on the earlier synced-data slice explicit enough about which invariants report generation may trust, such as deterministic ordering, explained zero-priced `SELL` admission, and validated holdings history? [Dependency, Assumption, Spec INT-001, Spec Assumptions]
- [ ] CHK031 Does the spec clearly separate what earlier sync normalization guarantees versus what report generation itself must validate again before calculating? [Dependency, Clarity, Spec INT-001, Plan Reporting Domain Design]
