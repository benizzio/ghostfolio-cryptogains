...
BUG-009 separates internal report-calculation precision from report rendering. Report math uses one shared 16-decimal internal precision with round-half-up handling for non-terminating divisions and proportional allocations, while Markdown output keeps the existing no-additional-rounding boundary rule.
...
**BUG-007 Note**: Single-activity currency selection requires an explicit tier currency before completeness validation. Tiers that carry order, asset-profile, or base financial values but no currency label are ineligible and must be skipped. Within the selected tier, same-tier derivation is allowed, and multiplication from quantity and unit price to gross value takes precedence over division-based unit-price derivation.

**BUG-008 Note**: Shared report-failure diagnostics must preserve the actionable `failure_detail` plus a separate ordered non-secret `failure_cause_chain` from the outer report failure to the deepest eligible wrapped cause. Nested causes follow the same redaction and suppression policy as other diagnostics content.

**BUG-009 Note**: Internal report calculations and derivations use one shared 16-decimal precision with round-half-up rounding for non-terminating divisions and proportional allocations. The generated Markdown reuses those internal results without an added report-boundary rounding step, so otherwise valid histories that need repeating-decimal math remain reportable.

**BUG-009-1 Note**: Follow-up implementation must preserve fragment-level liquidation matches with matched proceeds and matched gain or loss, apply rounded-intermediate formula ordering instead of direct proportional shortcuts where the artifacts define a per-unit intermediate, and remove pre-report exact unit-price division gates that reject otherwise valid gross-value-only rows before report generation.
...
- [x] BUG-008 diagnostics cause-chain preservation: Eligible report-failure diagnostics must keep actionable `failure_detail` and a separate ordered `failure_cause_chain`, and verification must prove deterministic outer-to-inner ordering plus nested-cause redaction or suppression for secret-bearing wrapped errors.
- [x] BUG-009 internal-calculation precision separation: Verification must prove that report rendering keeps its non-rounded boundary behavior while internal report calculations use one shared 16-decimal round-half-up precision for same-tier derivation, average-cost updates, partial-lot basis allocation, and proportional proceeds allocation, and that otherwise valid histories do not fail solely because a required division repeats.
- [x] BUG-009-1 implementation drift follow-up: Verification must prove fragment-level matched proceeds and matched gain or loss survive into the calculated report model, rounded-intermediate formula ordering is used where the spec defines per-unit intermediates, and pre-report sync validation does not reject otherwise valid gross-value-only rows solely because unit-price derivation repeats.
...
- Resolve monetary inputs per activity using a strict `order -> asset -> base` context selection. A tier is eligible only when it has an explicit currency code. Currencyless tiers are skipped even if they carry gross value, unit price, or fee fields. The first eligible tier that can supply or derive the complete monetary value set for that activity becomes the selected context. The calculator must not fill missing gross value, fee, or unit price from a different tier.
- When a selected tier lacks gross value but has positive quantity and unit price, derive gross value by same-tier multiplication before any division-based unit-price fallback. Unit-price derivation by division uses the shared 16-decimal internal calculation precision with round-half-up rounding.
...
- Keep all calculation state in `apd.Decimal`. Reuse `internal/support/decimal` for canonical exact-decimal parsing and formatting, and add report-local helpers for the shared 16-decimal internal calculation precision with round-half-up division and allocation handling only where that logic is genuinely report-specific. HIFO lot comparison should use cross multiplication where possible to avoid unnecessary division. Valid non-terminating divisions are rounded under that shared policy rather than failing the report.
- Preserve liquidation match fragments in the calculated report model when one priced liquidation spans multiple acquisition fragments. Matched proceeds and matched gain or loss use the rounded `proceeds_per_unit` intermediate defined by the spec before multiplication by matched quantity.
...
- Extend the shared report diagnostics context so `failure_detail` remains the actionable top-level summary and `failure_cause_chain` preserves the ordered non-secret wrapped causes starting at that outer failure and continuing to the deepest eligible cause. The runtime extraction path and shared diagnostics JSON writer must preserve wrapped causes instead of flattening them to one string.
- Reconcile the shared diagnostics contract described in `specs/004-source-faithful-diagnostics/spec.md` so synced-data diagnostics and report-failure diagnostics use the same source-faithful `failure_detail` plus ordered `failure_cause_chain` semantics when wrapped causes are eligible for serialization.
...
- Report-failure diagnostics JSON must serialize `failure_cause_chain` separately from `failure_detail`. The chain is ordered outer-to-inner and uses the same redaction and suppression rules, so nested causes that contain protected or secret-bearing text are redacted or omitted instead of written verbatim.
...
- Unit tests cover basis calculators for FIFO, LIFO, HIFO, Average Cost Basis, scope-local hybrid fallback and post-zero reset, zero-priced holding reductions, production-shaped explained zero-priced `SELL` rows with explicit zero `unit_price`, `gross_value`, and `fee_amount`, full-liquidation counts, single-activity currency selection, rejected-token unlock-screen rendering and field-reset behavior, canonical Markdown rendering, successful rounded non-terminating division at the shared 16-decimal internal calculation precision, filename suffixing, partial-file cleanup, shared diagnostic JSON serialization, original-persisted-record diagnostic capture, explicit `null` emission for nullable fields, and report-failure diagnostics redaction rules.
- Unit tests must also cover skipping tiers without explicit currency before completeness validation, same-tier gross-value derivation by multiplication, same-tier unit-price derivation at the shared 16-decimal internal calculation precision, and multiplication preference over division-based fallbacks within one selected explicit-currency tier.
- Verification must add wrapped-error cases for calculation, rendering, output-preparation, and diagnostics-generation failures, asserting that generated diagnostics keep `failure_detail`, ordered `failure_cause_chain`, deterministic cause order, and nested-cause redaction or suppression.
- Verification must add deterministic histories where same-tier derivation, average-cost updates, partial-lot basis allocation, or proportional proceeds allocation require repeating-decimal math, asserting successful 16-decimal round-half-up internal results and unchanged no-additional-rounding Markdown rendering.
- Verification must distinguish rounded-intermediate formula ordering from direct proportional shortcuts with a case such as disposing `2` units from `1` basis over `3` total quantity, and must prove gross-value-only activity rows with repeating `gross_value / quantity` pass Ghostfolio response validation, synced-history amount resolution, and report generation when same-tier requirements are otherwise met.
...
- BUG-007: Single-activity currency selection must separate tier eligibility from tier completeness. Higher-priority tiers that lack currency labels remain diagnostic context only and must not cause terminal failure. The implementation also needs same-tier derivation with multiplication precedence over division so production-shaped asset-tier inputs do not fail unnecessarily.
- BUG-008: Current report diagnostics preserve only one `failure_detail` string, so wrapped lower-level causes disappear from persisted artifacts. The implementation must keep both the actionable outer failure and a safe ordered wrapped-cause chain without leaking secret-bearing nested text.
- BUG-009: The current artifact set collapses report rendering precision into internal calculation precision. This fix must define one shared 16-decimal round-half-up internal calculation policy, apply it consistently across same-tier derivation and cost-basis math, and verify that otherwise valid repeating-decimal histories do not fail while Markdown output still adds no separate rounding step.
- BUG-009-1: The follow-up drift leaves fragment-level proportional proceeds out of the calculated report model, leaves pre-report exact unit-price division gates in sync validation, and leaves verification unable to distinguish rounded-intermediate allocation from direct proportional shortcuts.
