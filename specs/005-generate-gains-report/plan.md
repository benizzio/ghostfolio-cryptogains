...
**BUG-009 Constraint Clarification**: The stale constraint text above that says non-terminating required divisions fail is superseded by [BUG-009]. Internal report calculations now use one shared 16-decimal precision with round-half-up rounding for non-terminating divisions and proportional allocations, while Markdown output keeps the existing no-additional-rounding boundary rule.
