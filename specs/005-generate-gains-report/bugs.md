...
- [ ] it is currently very hard to understand calculation errors reports because the added error includes only the lower level of the call stack. We need to add a new field with the chain of error causes from `failure_detail` to better understand error reports
