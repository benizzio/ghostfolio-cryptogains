...
- [X] it is currently very hard to understand calculation errors reports because the added error includes only the lower level of the call stack. We need to add a new field with the chain of error causes from `failure_detail` to better understand error reports
- [X] currently the rule of not rounding report numbers is being extended to any calculations, meaning that divisions are limited to exact divisions, generating errors if the division is not exact. For complex calculations involving cost basis methods this is blocking the processing of some activities. We will keep the non-rounding rule for presenting report numbers in this slice, but for calculations we will allow rounding. The decimal precision internally should be of 16 decimals
- [ ] fix the concept of Single Responsibility Principle for go files also
- [ ] expand SRP to DDD layered architecture and clean architecture principles, to isolate infrastructure code from domain and application code
- [ ] DO IN ANOTHER SPEC - the application is currently keeping the token in memory in cleartext and a memory dump of the application process would reveal the token. We need to add a security layer to make sure this is not possible while still being able to keep and use the token for later requests (research the best method to do it)