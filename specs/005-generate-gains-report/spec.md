...
**Bugfix**: 2026-05-22 — [BUG-004] Clarified that unavailable `Sync and Reports` actions remain visible but are skipped by keyboard focus until they become enabled.

**Bugfix**: 2026-05-22 — [BUG-005] Restored the rule that snapshot miss alone cannot expose `Sync and Reports`, and defined rejected-token unlock handling on the unlock screen.

**Bugfix**: 2026-05-23 — [BUG-006] Added report-failure diagnostics requirements, source-faithful persisted activity context, and explicit `null` rendering for shared diagnostics artifacts.

**Bugfix**: 2026-05-23 — [BUG-007] Clarified that currencyless tiers are skipped during single-activity currency selection and that same-tier exact multiplication is preferred before division-based fallbacks.

**Bugfix**: 2026-05-23 — [BUG-008] Added ordered report-failure diagnostics cause-chain requirements and verification criteria.

**Bugfix**: 2026-05-23 — [BUG-009] Separated shared 16-decimal internal report-calculation precision from non-rounded report rendering and removed exact-division failure for otherwise valid histories.

**Bugfix**: 2026-05-24 — [BUG-009-1] Clarified authoritative rounded-intermediate allocation ordering and fragment-level matched-proceeds expectations for the BUG-009 follow-up drift.

## Terms Used In This Spec

- **Sync and Reports context**: The token-unlocked workflow reached from the main menu after either a selected-server protected snapshot unlocks or Ghostfolio authenticates the informed token as a new isolated local-user context. While this context remains active, the user can run `Sync Data` and `Generate Capital Gains Report` without informing the token again.
...
**Independent Test**: Start from the main menu with completed setup, open `Sync and Reports`, verify that a Ghostfolio-rejected token keeps the user on the unlock screen with `access denied`, `Unlock` disabled, and only `Back`, then re-enter with a token that unlocks a selected-server snapshot or authenticates as a new isolated local-user context, verify that the contextual menu shows `Sync Data`, `Generate Capital Gains Report`, and the correct synced-data readiness state, then enter `Sync Data` and verify that the screen explains token reuse without rendering token input.

**Acceptance Scenarios**:

1. **Given** setup is complete and the user is on the main menu, **When** the user selects `Sync and Reports`, **Then** the system requires the Ghostfolio security token before exposing sync or reporting actions, ~~and snapshot miss alone may still expose the context so the user can run `Sync Data`~~. Snapshot miss alone is superseded by [BUG-005]; the system opens the context only after either a selected-server protected snapshot unlocks or Ghostfolio authenticates that token as a new isolated local-user context.
2. ~~Given the token unlocks the active context and no synced activity data exists for that token and selected server, When the `Sync and Reports` menu is shown, Then both `Sync Data` and `Generate Capital Gains Report` are visible, `Sync Data` is available, report generation is unavailable with a clear reason, and keyboard navigation lands only on enabled actions.~~ Superseded by [BUG-005] because snapshot miss alone does not unlock the context.
3. **Given** no selected-server snapshot unlocks for the informed token and Ghostfolio rejects that token, **When** the unlock attempt completes, **Then** the system stays on the unlock screen, shows `access denied`, disables `Unlock`, keeps `Back` as the only available action, preserves the rejected token field value for that failed screen instance, and clears that field only after the user leaves with `Back` and later returns to the unlock screen.
4. **Given** the token unlocks the active context by unlocking a selected-server snapshot or by Ghostfolio authenticating that token as a new isolated local-user context, and no synced activity data exists for that token and selected server, **When** the `Sync and Reports` menu is shown, **Then** both `Sync Data` and `Generate Capital Gains Report` are visible, `Sync Data` is available, report generation is unavailable with a clear reason, and keyboard navigation lands only on enabled actions.
5. **Given** the token unlocks the active context and synced activity data exists, **When** the `Sync and Reports` menu is shown, **Then** `Sync Data` shows the last successful sync date and time and `Generate Capital Gains Report` is available.
6. **Given** the user completes a sync or report-generation workflow and remains inside the active unlocked context, **When** that workflow ends, **Then** the system returns to the `Sync and Reports` menu without requiring the token again.
7. **Given** the user selects `Sync Data` inside the active unlocked context, **When** the `Sync Data` screen is shown, **Then** the system does not ask for, display, focus, or allow editing of the Ghostfolio security token and shows only the action prompt needed to start sync.
...
1. **Given** synced data contains at least one reportable year, **When** the user selects a year and cost basis method and confirms generation, **Then** the system creates a yearly capital gains and losses report as a Markdown file in the user's Documents folder, uses the shared internal report-calculation precision from `FR-019b` for required internal derivations and allocations, requests the operating system to open it, and returns the user to the `Sync and Reports` menu.
2. **Given** the selected year has activity before, within, and after it, **When** the report is calculated, **Then** earlier activity is used to establish holdings and basis using the shared internal report-calculation precision from `FR-019b` where divisions or proportional allocations are required, only liquidations inside the selected year contribute gains and losses, and later activity is ignored.
...
11. **Given** the selected year and method require one priced activity whose monetary values cannot be supplied completely by any one explicit-currency activity currency context after skipping tiers without a currency code and allowing same-tier derivation under `FR-019b`, **When** the user starts generation, **Then** the attempt fails before any file is saved, the error is actionable and non-secret, the user remains inside the unlocked `Sync and Reports` context, and any generated diagnostics follow the report-failure diagnostics rules in `FR-034b` through `FR-035d`.
12. **Given** report generation fails during calculation, validation, rendering, or output preparation before the final Markdown file is saved, **When** the failure result is shown, **Then** the system supports generation of a separate local report-failure diagnostics artifact, asks whether to generate it outside explicit development mode, generates it automatically in explicit development mode, tells the user where it was written when it is generated, preserves the actionable `failure_detail` plus any ordered non-secret wrapped-cause chain under `FR-035d`, and does not use this failure path for otherwise valid histories that only require non-terminating internal division or allocation under `FR-019b`.
13. **Given** a report-generation failure is tied to one persisted activity record, **When** a diagnostics artifact is generated for that failure, **Then** it includes the original persisted activity record, renders absent source fields as explicit `null`, does not substitute selected activity-calculation inputs, rendered report rows, or other derived report values for that offending record, and keeps any ordered wrapped-cause data separate from that offending-record payload.
14. **Given** the selected year and method would render cross-activity monetary outputs from priced activities whose selected activity currency codes are not all the same, including runs where a lower-priority explicit-currency tier was selected because a higher-priority tier was currencyless, **When** the user starts generation, **Then** the calculations consider all currencies used in the calculation as equal and defines the shared report calculation currency as `NOT APPLICABLE` for this slice.
15. **Given** synced data preserves explicit zero-valued `unit_price`, `gross_value`, or `fee_amount` on an explained zero-priced holding reduction, **When** the report is generated, **Then** those preserved values remain available to render as `0` in that detail row, they do not create proceeds or gain-or-loss behavior, and they do not make an activity-currency context required for that row.
...
2. **Given** any supported cost basis method is selected, **When** the yearly report is generated, **Then** that one method is applied consistently to every included liquidation in that report run, including the shared internal report-calculation precision from `FR-019b` for internal derivations and allocations.
...
- A higher-priority priced-activity tier contains financial values but no explicit currency code, so that tier is skipped before completeness validation and selection continues to the first later tier with explicit currency and complete or exactly derivable same-tier values.
- A selected explicit-currency tier has positive quantity and unit price but no gross value, so gross value is derived by same-tier multiplication before any division-based unit-price fallback is considered.
- A valid report history requires non-terminating division for same-tier unit-price derivation, partial-lot basis allocation, proportional proceeds allocation, or average-cost updates, so report generation succeeds using the shared 16-decimal internal calculation precision from `FR-019b` and renders canonical strings from those internal results without an added report-boundary rounding step.
...
- The user's Documents location is unavailable or not writable at generation time, so the Markdown report is not saved but the workflow can still generate a separate diagnostics artifact under the report-failure diagnostics rules.
...
- A report-failure or synced-data diagnostics artifact includes an offending record with absent source fields, so those fields must be rendered as explicit `null` values instead of being omitted.
...
- **FR-002**: Selecting `Sync and Reports` MUST require the user to inform the Ghostfolio security token before the system exposes `Sync Data` or `Generate Capital Gains Report`. ~~If no selected-server snapshot unlocks, the context may still open immediately so the user can run `Sync Data`.~~ That snapshot-miss behavior is superseded by [BUG-005]. The system MUST expose those actions only after either a selected-server protected snapshot unlocks or Ghostfolio authenticates that token as a new isolated local-user context.
- **FR-002a**: If no selected-server snapshot unlocks for the informed token and Ghostfolio rejects that token, the system MUST stay on the unlock screen, show `access denied`, disable `Unlock`, leave `Back` as the only available action, preserve the rejected token field value for that failed screen instance, and clear that field only after the user leaves with `Back` and later returns to the unlock screen.
- **FR-003**: While the active `Sync and Reports` context remains unlocked, the system MUST present `Sync Data` and `Generate Capital Gains Report` as separate actions and MUST allow the user to move between them without informing the token again. A context counts as active only after `FR-002` succeeds by selected-server snapshot unlock or by Ghostfolio authentication of a new isolated local-user context. Inside that active context, the `Sync Data` workflow MUST use the stored runtime token and MUST NOT ask for, display, focus, or allow editing of the token again.
...
- **FR-005**: The system MUST keep `Generate Capital Gains Report` unavailable until synced data exists and at least one reportable year can be derived from that data for the active `Sync and Reports` context. While that action is unavailable, it MUST remain visible with its unavailable reason and MUST be skipped during keyboard navigation so selection focus stays on enabled actions only.
...
- **FR-019a**: The system MUST, for this slice, render report quantities and monetary values as canonical exact-decimal strings with no report-boundary rounding, trimming only non-significant formatting. This rendering rule applies only at the report output boundary; internal calculations follow `FR-019b`.
- **FR-019b**: The system MUST use one shared internal report-calculation precision of 16 decimal places for report calculations and derivations. When a required division or proportional allocation does not terminate exactly, the system MUST round that internal result to 16 decimal places using round half up before later calculations continue. For formulas that first derive a unit or per-fragment intermediate such as `lot_unit_cost`, `average_unit_cost`, `matched_unit_cost_i`, or `proceeds_per_unit`, the system MUST apply this precision to the division before the later multiplication and MUST NOT replace the formula with a direct proportional shortcut when that shortcut changes the 16-decimal internal result. Rendered report values reuse those internal results without any additional report-boundary rounding step.
...
- **FR-031**: When monetary amounts are needed for calculations within one activity, the system MUST evaluate currency contexts in this priority order: transaction order currency, asset currency, then portfolio base currency, and select the first context that has an explicit currency code and provides or can ~~exactly~~ derive the complete required monetary value set for that activity using only values from that same tier. A tier without an explicit currency code is not a selectable currency context and MUST be skipped before completeness validation or derivation is attempted.
...
- **FR-031c**: ~~Within a selected explicit-currency context for a priced activity, the system MAY derive a missing unit price from gross value and positive quantity only when that division terminates exactly under `FIN-001` and every derivation input comes from that same selected tier; otherwise the report attempt MUST fail.~~ Superseded by [BUG-009]. Within a selected explicit-currency context for a priced activity, the system MAY derive a missing unit price from gross value and positive quantity only when every derivation input comes from that same selected tier. If that division does not terminate exactly, the derived unit price MUST use the shared internal report-calculation precision from `FR-019b` before later calculations continue.
- **FR-031d**: Within a selected explicit-currency context for a priced activity, if gross value is missing but positive quantity and unit price are present in that same tier, the system MUST derive gross value by multiplication before considering any division-based fallback. The system MUST NOT mix tiers to complete that derivation.
...
- **FR-034**: If no explicit-currency context in the priority order can supply or ~~exactly~~ derive the complete set of monetary values needed for a calculation after skipping tiers without a currency code, the system MUST fail the report generation attempt with an actionable error instead of mixing currency tiers within that activity.
...
- **FR-034b**: When report generation fails before the final Markdown file is saved because report validation, calculation, rendering, or output preparation cannot complete safely, the system MUST support generation of a separate local diagnostic report for that attempt. When the application is not running in explicit development mode, the system MUST ask the user whether to generate that report before writing it. When the application is running in explicit development mode, the system MUST generate the report automatically for the same eligible failure categories and MUST tell the user where it was written whenever it is generated. A successfully saved Markdown report whose later automatic-open request fails is not an eligible report-generation failure for this diagnostics flow.
...
- **FR-035a**: Report generation MUST revalidate report-slice calculation preconditions that earlier sync may not have guaranteed, including positive quantity for priced activity rows, explicit currency presence before a tier can be selected, ~~exact-division termination where required by the selected method,~~ chosen-context completeness, ~~same-tier derivation exactness,~~ correct application of the shared internal report-calculation precision from `FR-019b` for non-terminating divisions, same-tier derivations, and proportional allocations, shared report-calculation-currency availability where rendered cross-activity monetary outputs are required, and the selected method's ability to allocate basis through the selected-year cutoff. If any such precondition fails, report generation MUST fail safely under `FR-035`.
- **FR-035b**: A report-failure diagnostic report MUST be stored only as a separate local troubleshooting artifact and MUST exclude the Ghostfolio security token, bearer JWT, raw unprotected payload fragments, transient user-visible report-failure messages, and any reusable token verifier. Outside explicit development mode, the report MUST redact quantity, unit price, gross value, fee value, and any other financial-value fields from included offending-record details. In explicit development mode, it MAY include the full non-secret offending-record context needed for troubleshooting.
- **FR-035c**: When a report-failure diagnostic report includes activity-specific context, it MUST serialize the original persisted `ActivityRecord` data that triggered the failure, not selected single-activity calculation inputs, rendered report rows, or other derived report values. Absent source fields in that diagnostic record MUST be rendered explicitly as `null`. The same explicit `null` rendering rule MUST also apply to synced-data diagnostic reports that use the shared diagnostic artifact format.
- **FR-035d**: A report-failure diagnostic report MUST preserve the existing actionable non-secret `failure_detail` and MUST also serialize a separate ordered `failure_cause_chain` field for the same failure. That cause chain MUST preserve deterministic outer-to-inner order starting with the actionable report failure and continuing toward the deepest eligible non-secret wrapped cause, MUST apply the same redaction and suppression rules as the rest of the diagnostics artifact, and MUST redact or omit any nested cause entry that would expose tokens, JWTs, raw payload fragments, reusable token verifiers, or production-disallowed financial-value fields.
...
- for each in-year liquidation, the disposed quantity, allocated basis, net liquidation proceeds, gain or loss, and, when the liquidation spans multiple matched acquisition fragments, each fragment's matched quantity, matched basis, matched proceeds, and matched gain or loss
...
Every division-based or proportional internal result in this section and the method sections below uses the shared internal report-calculation precision from `FR-019b`. When one of those results does not terminate exactly, it is rounded to 16 decimal places using round half up before later calculations continue. Formula order is authoritative: a division shown before multiplication produces the rounded internal intermediate before that multiplication is applied.
...
A tier without an explicit currency code is not a selectable currency context and must be skipped before checking whether that tier can provide or derive the required monetary values.
...
- if the chosen context cannot provide or ~~exactly~~ derive every monetary value needed from that activity using only that same tier, the report-generation attempt fails instead of mixing tiers
...
Within a selected explicit-currency tier, same-tier derivation is allowed when needed to complete the priced activity input. When quantity and unit price are present but gross value is missing, the system derives gross value by multiplication first. Division-based derivation, such as deriving unit price from gross value and positive quantity, is used only when the needed unit measure is absent and the derived result uses the shared internal report-calculation precision from `FR-019b`.
...
- **SEC-001**: The Ghostfolio security token MUST be informed explicitly by the user to enter `Sync and Reports`, kept only for the active unlocked context, reusable for sync and reporting actions while that context remains open, cleared when the user leaves that context or the application ends, excluded/unreadable from logs, output, diagnostics, and persisted artifacts, and not re-rendered or exposed as editable input by the in-context `Sync Data` workflow after unlock. A token that unlocks no selected-server snapshot and is rejected by Ghostfolio MUST NOT activate the context; its informed value may remain only on the failed unlock-screen instance until the user leaves with `Back`, after which the field is cleared before the next entry.
- **SEC-002**: Before the final Markdown file is saved to the user's Documents folder, report inputs, intermediate calculations, rendered content, and any temporary storage MUST remain inside the same security boundary and protection level used for synced financial and user-linked data, and they MUST be kept out of unprotected temporary locations. If report generation fails before final save, that same boundary applies until any separate diagnostics artifact is intentionally written under `FR-034b` through `FR-035c`.
- **SEC-003**: The final Markdown file saved in the Documents folder is intentionally outside the application's protected-storage boundary. After that file is saved and handed to the operating system, the application MUST retain no additional cleartext copy of the report and MUST leave no recoverable cleartext temporary residue under application-managed storage. User removal of that persisted cleartext output is by deleting the saved Markdown file from the Documents folder because this slice keeps no report catalog or additional retained copy. A report-failure diagnostics artifact is a separate troubleshooting file, not a retained successful-report copy, and MUST follow the secret-exclusion, redaction, and path-disclosure rules in `FR-034b` through `FR-035c`.
- **FIN-001**: All quantities, proceeds, fees, allocated basis, gains, and losses MUST use exact decimal arithmetic, ~~preserve source precision until final rendering,~~ preserve source-provided precision until internal report calculations or derivations require the shared 16-decimal calculation policy from `FR-019b`, include fees in acquisition basis or liquidation proceeds as applicable, treat explained zero-priced holding reductions as zero-gain and zero-loss events, show losses with a negative sign in final report output, and, for this slice, render report values as canonical exact-decimal strings with no report-boundary rounding and only non-significant formatting trimmed.
- **FIN-002**: Within one activity, calculations MUST use the single-activity currency context defined in `Single-Activity Currency Context Definitions`, including the priority `order -> asset -> base`, the rule against mixing currency tiers inside one activity, the rule that tiers without an explicit currency code are skipped before completeness validation, and the requirement to fail instead of mixing tiers when no explicit-currency context can safely supply or ~~exactly~~ derive the required same-tier values. After values enter cross-activity cost basis and gains-and-losses calculations, the system MUST preserve the selected activity currency code for priced activity inputs, MUST require one shared report calculation currency under `FR-028` for rendered cross-activity monetary outputs, and MUST perform no currency conversion in this slice.
- **QUAL-001**: Automated validation MUST cover main-menu entry into `Sync and Reports`, token gating and token reuse within the active unlocked context, selected-server snapshot unlock, authenticated new-context unlock after snapshot miss, rejected-token unlock refusal with `access denied`, blocked `Unlock`, back-only state, and deferred token-field clearing only after leaving and re-entering the unlock screen, in-context `Sync Data` token reuse without any visible or editable token input, last-sync timestamp display beside `Sync Data`, disabled-action keyboard navigation that skips unavailable `Sync and Reports` actions, available-year selection, each supported cost basis method, yearly gains-and-losses calculations that count only liquidations inside the selected year, ignoring assets first acquired after the selected year, reference-section liquidation counts for reopened assets, report section ordering and definitions, negative-sign rendering for losses, zero-result assets, zero-priced holding reductions carried forward from the synced-data rules, preserved explicit zero-valued explained zero-priced holding-reduction fields distinguished from missing values, single-activity currency-context selection, no cross-tier mixing, report-calculation-currency determination, Documents-folder save behavior, filename uniqueness and alphabetical sortability, automatic-open request success and failure behavior, absence of report history, protected handling of pre-save report content, report-failure diagnostics eligibility for calculation, validation, rendering, and output-preparation failures, production prompt and explicit-development-mode automatic generation, diagnostics-path disclosure, activity-specific original persisted-record context, explicit `null` rendering for absent source fields in both report and synced-data diagnostics, opener-only warning distinction after a successful save, and the repository's full coverage gates using integration-first tests and only targeted unit tests for complex calculation rules.
- **QUAL-001a**: Automated validation MUST include priced-activity currency-context regressions where a higher-priority tier contains financial fields but no currency code, a later tier with an explicit currency can supply or ~~exactly~~ derive the complete same-tier monetary value set, same-tier multiplication derives gross value before any division-based fallback is considered, division-based derivation uses the shared internal report-calculation precision from `FR-019b`, and terminal failure occurs only after all explicit-currency tiers are unusable.
- **QUAL-001b**: Automated validation MUST include wrapped-error diagnostics regressions proving that eligible report-failure diagnostics preserve both `failure_detail` and the ordered `failure_cause_chain`, keep outer-to-inner cause ordering deterministic, and redact or suppress nested secret-bearing causes under the same diagnostics policy used for top-level failure details.
- **INT-001**: The feature depends on the existing synced activity dataset from earlier slices, including available report years, scope reliability, and the explained zero-priced `SELL` records admitted by `specs/003-store-activity-data/spec.md`, which may preserve explicit zero-valued source fields even though report calculation does not require them, and on operating-system services that provide a writable personal Documents location and a default Markdown-file association. This slice introduces no external currency-conversion dependency. Report-failure diagnostics reuse the application-owned diagnostics artifact flow introduced by `specs/003-store-activity-data/spec.md`, including its explicit-development-mode policy and local diagnostics directory.
- **INT-001a**: Report generation MAY trust from the earlier synced-data slice only these invariants: supported activity types already limited to `BUY` and `SELL`, stable stored asset identity keys preserved from upstream `SymbolProfile.id`, `available_report_years` already derived from source timestamps, deterministic same-asset order already established from source calendar date then `BUY` before `SELL` then `source_id`, explained zero-priced `SELL` rows already carrying their required explanation and possibly preserved explicit zero-valued source fields such as `unit_price`, `gross_value`, and `fee_amount`, running quantity already validated not to drop below zero during synced-data replay, and scope reliability already classified. When report-failure diagnostics need activity-specific context, the protected-cache `ActivityRecord` remains the source of truth for that context.
- **INT-001b**: Report generation MUST still validate report-run preconditions that are method-specific or output-specific, including selected-year membership, positive priced-activity quantity, explicit-currency-tier eligibility before context selection, chosen-context completeness, ~~exact-division termination, same-tier derivation exactness,~~ correct application of the shared internal report-calculation precision from `FR-019b`, shared report-calculation-currency availability where rendered cross-activity monetary outputs are required, method-specific basis-allocation ability, and output-path writability. Eligible report failures MUST also preserve enough failure context to drive the diagnostics flow in `FR-034b` through `FR-035c`, including the original offending persisted record when one exists.
- **INT-001c**: If report-generation code wraps lower-level calculation, rendering, output-preparation, or diagnostics-generation failures with `%w` or equivalent cause preservation, the diagnostics flow MUST carry that ordered non-secret cause chain into `failure_cause_chain` rather than flattening all wrapped causes into `failure_detail`.
...
- **Report Failure Diagnostic Report**: The separate local troubleshooting artifact that can be written for eligible report-generation failures, carrying failure category detail, generated path disclosure, and when applicable the original persisted activity record with explicit `null` source fields.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: 100% of active `Sync and Reports` menu states show both `Sync Data` and `Generate Capital Gains Report`, 100% of states without reportable synced data keep report generation unavailable with a clear reason, and 100% of those unavailable states skip the disabled report action during keyboard navigation so selection lands only on enabled actions.
...
- **SC-003**: For controlled multi-year ledgers with known expected outcomes, 100% of yearly per-asset results and yearly totals match the expected values across all five supported cost basis methods, count only liquidations inside the selected year, ignore assets first acquired after the selected year, show losses with a negative sign, include scope-local hybrid scenarios covering exact matching that remains exact, first fallback activation in one applicable scope, continued fallback until that scope reaches zero, and same-scope post-liquidation reset after later reacquisition, and include deterministic histories that require rounded internal division at the shared 16-decimal calculation precision from `FR-019b` for average-cost updates, partial-lot basis allocation, fragment-level proportional proceeds allocation, or same-tier derivations, including at least one case that distinguishes rounded-intermediate formula ordering from direct proportional allocation.
- **SC-003a**: 100% of priced-activity currency-context regression fixtures skip higher-priority tiers that lack an explicit currency code, select the first later explicit-currency tier that can supply or ~~exactly~~ derive the required same-tier monetary values, prefer multiplication-based gross-value derivation over division-based fallbacks, apply the shared 16-decimal internal calculation precision from `FR-019b` where division is required, and fail only when every explicit-currency tier is unusable.
...
- **SC-008**: 100% of inspected application-managed storage and temporary artifacts created during report generation keep cleartext report content out of persistent application storage before final save and leave no leftover cleartext temporary files after success or failure. For eligible report-generation failures that produce diagnostics, 100% of generated diagnostics artifacts disclose their local path, exclude tokens, JWTs, raw payload fragments, reusable token verifiers, and production-disallowed financial-value fields, and serialize absent source fields as explicit `null`.
- **SC-008a**: 100% of eligible report-generation diagnostics created from wrapped failures preserve both the actionable `failure_detail` and a separate ordered `failure_cause_chain` that starts at the outer report failure and ends at the deepest eligible non-secret wrapped cause, with nested secret-bearing causes redacted or suppressed.
...
- A single-activity currency context is eligible only when that tier has an explicit currency code. Tiers without currency are skipped, and report generation fails only when no explicit-currency tier can supply or ~~exactly~~ derive the required same-tier monetary values without mixing tiers inside that activity.
