...
- **FR-019b**: The system MUST use one shared internal report-calculation precision of 16 decimal places for report calculations and derivations. When a required division or proportional allocation does not terminate exactly, the system MUST round that internal result to 16 decimal places using round half up before later calculations continue. Rendered report values reuse those internal results without any additional report-boundary rounding step.
...
Every division-based or proportional internal result in this section and the method sections below uses the shared internal report-calculation precision from `FR-019b`. When one of those results does not terminate exactly, it is rounded to 16 decimal places using round half up before later calculations continue.
