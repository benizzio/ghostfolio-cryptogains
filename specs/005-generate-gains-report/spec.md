...
- **QUAL-001a**: Automated validation MUST include priced-activity currency-context regressions where a higher-priority tier contains financial fields but no currency code, a later tier with an explicit currency can supply or exactly derive the complete same-tier monetary value set, same-tier multiplication derives gross value before any division-based fallback is considered, and terminal failure occurs only after all explicit-currency tiers are unusable.
