...
6. **Given** a report contains converted monetary values, **When** the user reviews conversion details, **Then** the report shows the source currency, report base currency, activity date, rate date, ~~rate authority,~~ rate value, original amount, and converted amount for each converted priced activity or for an equivalent per-activity audit section. BUG-004 supersedes repeating provider-level authority in per-amount audit rows; rate authority and rate kind are disclosed once in `Rate Source Summary`, and the `Currency Conversion Audit` table uses the compact column order `Date`, `Source ID`, `Asset`, `Amount Kind`, `Rate Date`, `Source Currency`, `Original Amount`, `Report Base Currency`, `Converted Amount`, `Quote Direction`, and `Rate Value`.
...
- **Conversion Audit Entry**: The report-visible evidence that connects an original activity amount, selected currency, activity date, ~~authority source,~~ rate date, rate value, quote direction, and converted amount. It may retain authority and rate-kind evidence for validation and source-summary correlation, but the rendered audit table excludes `Rate Authority` and `Rate Kind` columns.
