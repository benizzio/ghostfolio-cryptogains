...
- [ ] T067 [P] [US2] ⚠️ Reopened Add explicit zero-valued monetary field conversion tests proving valid zero fees and amounts remain zero and do not become report-visible zero-to-zero audit amount items in `internal/report/calculate/calculator_internal_test.go` (reopened — BUG-005)
...
- [ ] T082 [P] [US2] Add Markdown contract regression proving a source activity with unit price, gross value, and zero fee renders one conversion audit row with non-zero amount kinds grouped and no zero-to-zero fee slot in `tests/contract/markdown_report_contract_test.go`
- [ ] T083 [P] [US2] Add renderer unit regression for grouped converted amount display and zero-to-zero amount omission in `internal/report/markdown/renderer_internal_test.go`
...
- [ ] T045 [US2] ⚠️ Reopened ~~Render converted activity audit table in `internal/report/markdown/renderer_currency.go`~~ Render grouped converted activity audit table with one row or equivalent subsection per converted source activity, no `Rate Authority` or `Rate Kind` columns, and no zero-to-zero amount items in `internal/report/markdown/renderer.go` (reopened — BUG-005)
...
- [ ] T084 [US2] Update report model validation to keep `ConversionAuditEntry` grouped per converted source activity without requiring report-visible rows for zero-to-zero converted amount slots in `internal/report/model/capital_gains_report.go` and `internal/report/model/conversion_audit.go`
- [ ] T085 [US2] Verify report artifact creation records one `ConversionAuditEntry` per converted source activity with grouped `ConvertedActivityAmount` values in `internal/report/calculate/artifacts.go`
