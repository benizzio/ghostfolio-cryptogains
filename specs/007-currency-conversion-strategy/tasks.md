...
## Phase 8: Coding Standards Drift Remediation Follow-up

**Purpose**: Remediate current coding-standards drift findings recorded in `coding-standards-drift-report.md` after the previous drift-remediation phase is complete.

- [X] T102 Remediate `CODE-STAND-DRIFT-005` (Medium): Report Construction Bypasses Conversion Artifact Validation by keeping `RateSources` and `ConversionAuditEntries` inside the report-domain construction and validation boundary instead of assigning validation-sensitive fields after `NewCapitalGainsReport` returns, then update report calculation and any affected rendering callers in `internal/report/model/capital_gains_report.go`, `internal/report/calculate/calculator.go`, and adjacent report-domain paths; source: `coding-standards-drift-report.md#code-stand-drift-005-report-construction-bypasses-conversion-artifact-validation`; evidence: `internal/report/model/capital_gains_report.go:15-54`, `internal/report/model/capital_gains_report.go:91-121`, `internal/report/calculate/calculator.go:128-147`
- [X] T103 Remediate `CODE-STAND-DRIFT-006` (Medium): Runtime Parses Conversion Failure Context From Error Text by propagating explicit typed conversion failure context from currency integration through report calculation to runtime failure copy, replacing runtime parsing of `Error()` strings and token fragments in `internal/integration/currency/errors.go`, `internal/report/calculate/errors.go`, `internal/app/runtime/report_service.go`, and `internal/app/runtime/report_failure_context.go`; source: `coding-standards-drift-report.md#code-stand-drift-006-runtime-parses-conversion-failure-context-from-error-text`; evidence: `internal/integration/currency/errors.go:43-55`, `internal/integration/currency/errors.go:102-129`, `internal/report/calculate/errors.go:57-69`, `internal/app/runtime/report_service.go:238-295`, `internal/app/runtime/report_failure_context.go:83-144`
- [X] T104 Remediate `CODE-STAND-DRIFT-007` (Medium): Conversion Audit Model File Combines Multiple Domain Entities And Display Helpers by splitting `internal/report/model/conversion_audit.go` into cohesive report-model files for exchange-rate evidence, converted activity amounts, conversion audit entries, provider display labels, and related validation without changing the public domain behavior described in `specs/007-currency-conversion-strategy/data-model.md`; source: `coding-standards-drift-report.md#code-stand-drift-007-conversion-audit-model-file-combines-multiple-domain-entities-and-display-helpers`; evidence: `internal/report/model/conversion_audit.go:15-97`, `internal/report/model/conversion_audit.go:99-128`, `internal/report/model/conversion_audit.go:130-230`, `internal/report/model/conversion_audit.go:336-375`, `specs/007-currency-conversion-strategy/data-model.md:258-368`
- [X] T105 Verify `CODE-STAND-DRIFT-005`, `CODE-STAND-DRIFT-006`, and `CODE-STAND-DRIFT-007` remediation by running `go test ./internal/report/model ./internal/report/calculate ./internal/report/markdown ./internal/integration/currency ./internal/app/runtime ./tests/integration` from the repository root after the refactors; source: `coding-standards-drift-report.md#findings`
- [X] T106 Verify `CODE-STAND-DRIFT-005`, `CODE-STAND-DRIFT-006`, and `CODE-STAND-DRIFT-007` remediation with the maintained coverage command `make coverage` from the repository root and confirm the coverage artifacts remain under `dist/coverage/`; source: `coding-standards-drift-report.md#findings`
...
- **Coding Standards Drift Remediation Follow-up (Phase 8)**: Depends on the completed Phase 7 remediation tasks and the current `coding-standards-drift-report.md` findings.
