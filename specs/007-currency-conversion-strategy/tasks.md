...
- [X] T078 [P] [US2] Add Markdown contract assertion for exact `Currency Conversion Audit` compact header order in `tests/contract/markdown_report_contract_test.go`
- [X] T079 [P] [US2] Add renderer unit assertion for compact audit row order and absence of `Rate Authority` and `Rate Kind` columns in `internal/report/markdown/renderer_internal_test.go`
...
- [X] T045 [US2] ⚠️ Reopened ~~Render converted activity audit table in `internal/report/markdown/renderer_currency.go`~~ Render compact converted activity audit table with BUG-004 column order and no `Rate Authority` or `Rate Kind` columns in `internal/report/markdown/renderer.go` (reopened — BUG-004)
