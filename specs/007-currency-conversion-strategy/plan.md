...
**Storage**: No new exchange-rate persistence. Existing protected snapshots remain token-derived encrypted local data. Rate evidence is held in memory for one report run and then disclosed only in the generated cleartext Markdown report saved under the existing report-output contract.

**Testing**: Go `testing` with contract, integration, and unit suites. External provider behavior is covered with deterministic `httptest.Server` fixtures and golden response snippets, not live ECB or Federal Reserve calls. Coverage remains enforced with `go test ./... -covermode=atomic -coverprofile=coverage.cov` and `gocoverageplus`/repository coverage gates.
...
- [x] Testing: Integration-first coverage will verify TUI report base-currency selection, runtime generation, official provider anticorruption adapters with mocked provider responses, Markdown audit sections, failure handling, and no-partial-file cleanup. Targeted unit tests are justified for quote-direction formulas, prior available date selection, canonical model validation, and redaction-sensitive failure shaping. Coverage gates remain mandatory.
- [x] Empirical financial validation: Existing empirical financial tests under `tests/empirical/` and data under `testdata/empirical/` remain read-only and single-currency. New conversion coverage uses project-owned contract, integration, and unit fixtures.
...
- [x] Architecture: Report-domain conversion rules live under `internal/report/`. Provider IO and payload mapping are isolated behind an anticorruption layer so `internal/report/calculate/` consumes canonical rate evidence through an interface rather than provider DTOs. TUI only captures the report base-currency choice. Runtime coordinates dependencies without embedding conversion math.
...
internal/
├── app/
│   └── runtime/              # Compose report service, exchange-rate service, diagnostics, and output lifecycle
├── report/
│   ├── model/                # Report request base currency, canonical rate evidence, converted amounts, audit entries, validation
│   ├── exchange/             # ECB/Federal Reserve provider adapters and canonical anticorruption layer
...
**Structure Decision**: Keep the feature inside the existing single Go module. Use a new `internal/report/exchange/` package for provider-specific HTTP and payload mapping because the integration exists only to support report-domain conversion. Keep canonical conversion evidence in `internal/report/model/`, calculation orchestration in `internal/report/calculate/`, and user interaction in `internal/tui/`. Do not move provider DTOs or network details into TUI, runtime diagnostics, or cost-basis state implementations.
...
- Revision behavior: use the currently published ECB production observation at report generation time and disclose the rate date and value.
...
- Revision behavior: use the currently published H.10/DDP data at report generation time. Past release archive pages are not the source of truth for regenerated reports because they may not reflect subsequent revisions.
...
5. If the selected activity currency differs from the report base currency, resolve canonical rate evidence by source currency, base currency, and source-calendar activity date.
6. Convert every selected monetary value that can affect cost basis, proceeds, fees, gains, losses, or totals before it enters basis state or liquidation calculations.
...
Provider adapters must expose a canonical report-domain interface that returns rate evidence independent of provider payload shape. The canonical model includes source currency, base currency, activity date, rate date, authority, provider, rate kind, quote direction, rate value, and source URL or dataset identity. ECB and Federal Reserve DTOs do not cross into `internal/report/calculate/`, `internal/report/basis/`, TUI, or Markdown rendering.

The issue requirement for scalability is handled by keeping provider selection behind a base-currency registry. Adding a future base currency should require adding one provider adapter and registry entry, not changing cost-basis methods or report rendering logic beyond supported currency labels.
...
- Provider unavailability fails before final save unless the required evidence is already present in the current in-memory report run.
...
- Integration tests run report generation with mocked ECB and Federal Reserve provider responses, including same-currency rows, converted rows, mixed source currencies, prior-business-date fallback, provider outage, unsupported currency, malformed currency, and no-partial-file cleanup.
- Unit tests cover ECB EXR mapping, Federal Reserve H.10 quote-direction mapping, canonical rate evidence validation, conversion formulas, 16-decimal division bounding, in-memory per-run rate reuse, and redaction-safe failure construction.
