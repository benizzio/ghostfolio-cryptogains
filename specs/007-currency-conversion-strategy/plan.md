...
│   ├── markdown/             # Report currency header, provider-level source summary, and per-activity conversion audit rendering
...
10. Render `Asset Detail` `In-Year Activity` columns in the BUG-007 order with `Original Activity Currency`; render `Liquidation Calculations` without an `Activity Currency` column.
...
Asset detail rendering must keep original selected currency disclosure in the `In-Year Activity` table as `Original Activity Currency` and must not duplicate that disclosure as an `Activity Currency` column in `Liquidation Calculations`.
...
- Regression tests verify exact `Asset Detail` `In-Year Activity` column order with `Original Activity Currency` and verify `Liquidation Calculations` omits `Activity Currency`.
...
BUG-007 adds a rendering clarity edge case where asset detail tables can duplicate or ambiguously name activity currency. The simpler approach is to keep original selected currency disclosure in `In-Year Activity`, make the column name explicit, and omit the repeated liquidation column.
