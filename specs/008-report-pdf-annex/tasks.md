...
| WU29 | Phase 5: User Story 3 - Review Annex 1 Audit Evidence (Priority: P3) | T038 | Add fail-first 10,000 cached-activity Markdown/PDF annex scale integration test in `tests/integration/report_performance_flow_test.go`. | WU24 | `tasks.md`, `quickstart.md`, `contracts/report-rendering.md`, `contracts/report-output.md` | Inspect `git diff -- tests/integration/report_performance_flow_test.go`; run `go test ./tests/integration -run Performance`; confirm generated scale fixtures stay project-owned and `testdata/empirical/` remains unchanged. |
...
| WU40 | Phase 8: Test Coverage Drift Remediation | T080, T081, T082, T083, T084, T085, T086 | Remediate `COV-DRIFT-001` through `COV-DRIFT-006` in task order, then validate the required contract, integration, generated-PDF, large-history, cleanup, full-suite, and coverage-gate evidence. | WU39 | `tasks.md`, `test-coverage-drift-report.md`, `plan.md`, `quickstart.md`, `Makefile`, `.cov.json` | Inspect each remediation against its linked report topic; run the targeted commands while implementing T080-T085, then run every command in T086 and confirm the maintained coverage command executes the passing both-format 10,000-activity scenario. |
...
- [ ] T085 [COV-DRIFT-006] [Medium] Implement the "Renderer-Failure Cleanup Evidence Remains Unit-Only" remediation from `test-coverage-drift-report.md#cov-drift-006-renderer-failure-cleanup-evidence-remains-unit-only`: add a runtime-backed renderer-failure journey to `tests/integration/report_failure_flow_test.go` using a deterministic injectable renderer failure equivalent to the seam covered by `internal/app/runtime/report_service_internal_test.go`, then assert the failure result, empty output directory, absence of partial Markdown or PDF files, and no opener request; expose only the smallest testable runtime assembly seam needed under `internal/app/runtime/`
- [ ] T086 [COV-DRIFT-001] [COV-DRIFT-002] [COV-DRIFT-003] [COV-DRIFT-004] [COV-DRIFT-005] [COV-DRIFT-006] Validate all remediation topics in `test-coverage-drift-report.md#findings` with `gofmt` on changed Go files, `GHOSTFOLIO_CRYPTOGAINS_RUN_PERFORMANCE=1 go test ./tests/integration -run '^TestReportPerformanceFlowLargeHistoryFixture$' -count=1 -v`, targeted `go test` runs for `./internal/report/pdf`, `./tests/contract`, and `./tests/integration`, followed by `make test`, `make coverage`, and `make quality QUALITY_BASE_REF=origin/main`; inspect `dist/coverage/coverage.out` and `dist/coverage/coverage.xml` against `.cov.json` and confirm the maintained coverage run no longer skips the required both-format large-history scenario
