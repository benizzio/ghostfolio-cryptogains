...
As a user auditing the report, I want Annex 1 to contain detailed per-asset activity evidence and currency conversion evidence so that I can trace the main report totals back to historical records through the report year.
...
**Independent Test**: Can be tested by generating a report with multiple assets, historical activity before the report year, report-year activity, liquidations, gains or losses, and currency conversions, then verifying Annex 1 content and placement in both output formats.
...
3. **Given** the per-asset audit section is rendered, **When** historical activity through the report year is inspected, **Then** each included activity shows the activity details, post-activity held quantity, cost-basis effects after the activity, full liquidation events, and gains or losses triggered by the activity.
...
- If an asset has both report-year activity and historical activity before the report year, its main Asset Detail must keep the normal report-year structure, while Annex 1 includes the historical activity needed to audit the report through the report year.
...
- **FR-018**: The detailed per-asset audit report MUST list historical activity through the report year for each reported asset.
...
- **Per-Asset Audit Section**: A section of Annex 1 that groups historical activity evidence for one asset through the report year.
...
- **SC-008**: Annex 1 allows a reviewer to trace 100% of reported asset activity through the report year to post-activity held quantity, cost-basis effect, full liquidation status, and gain or loss effect.
