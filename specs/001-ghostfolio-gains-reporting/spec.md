...
13. **Given** the activity history contains a transfer between user-owned scopes, **When** the report is calculated, **Then** the application carries forward quantity, basis, acquisition timestamp, and provenance into the receiving scope and does not realize gain or loss for the transfer.
...
- A transfer between user-owned scopes must carry forward basis and provenance without realizing gain or loss.
...
- **FR-036**: The system MUST treat transfers between user-owned scopes as non-disposal movements that carry forward quantity, basis, acquisition timestamp, and exact-unit or deemed-disposal provenance into the receiving scope, and MUST not realize gain or loss for those movements.
...
- **Activity Record**: A timestamped asset acquisition, disposal, or movement event, including the data needed to determine holdings, basis, proceeds, fees, and any available source holding scope used to derive `applicable_scope`.
