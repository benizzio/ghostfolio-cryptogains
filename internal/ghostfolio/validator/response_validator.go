...
func deriveRoundedUnitPrice(grossValue apd.Decimal, quantity apd.Decimal) (apd.Decimal, error) {
	if _, err := decimalsupport.CanonicalString(grossValue); err != nil {
		return apd.Decimal{}, fmt.Errorf("prepare gross value for unit-price derivation: %w", err)
	}
	if _, err := decimalsupport.CanonicalString(quantity); err != nil {
	...
	var scaledQuotient = scaledRoundedQuotient(grossValue, quantity, validatorUnitPriceScale)
	...
	return apd.Decimal{
		Form:     apd.Finite,
		Negative: negative,
		Exponent: -validatorUnitPriceScale,
		Coeff:    coefficient,
	}, nil
}
